...
function Overview({ platform, leads, onTab }: { platform: PlatformDashboard; leads: AdminDashboard; onTab: (tab: Tab) => void }) { return <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
  ...
  <Panel title="تشغيل المطعم" icon={<ExternalLink className="size-5" />} text="تشغيل المنيو والطلبات الخاصة بالمطعم يبقى داخل مساحة العميل، وليس مركز المنصة." action="فتح مساحة العميل" onClick={() => void navigate({ to: "/owner" })} />
</div>; }
