...
function PlatformAdminPage() {
  ...
  return <main className="mx-auto grid max-w-[1500px] gap-5 py-4 lg:py-8">
    <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
      ...
      <div className="flex flex-wrap gap-2"><button type="button" onClick={() => setTab("leads")} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-xl border border-line bg-paper px-4 py-2 text-sm font-medium text-ink"><ClipboardList className="size-4" />اعتماد العملاء الجدد</button><Button variant="outline" onClick={() => { void load(); if (tab === "orders") void loadOrders(); }} disabled={loading || ordersLoading}><RefreshCw className={cn("size-4", (loading || ordersLoading) && "animate-spin")} /> تحديث البيانات</Button></div>
    </header>
...
function Overview({ platform, leads, onTab, onApproval }: { platform: PlatformDashboard; leads: AdminDashboard; onTab: (tab: Tab) => void; onApproval: () => void }) { return <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
...
function Leads({ leads, status, query, setStatus, setQuery }: { leads: AdminDashboard; status: LeadStatus | "all"; query: string; setStatus: (v: LeadStatus | "all") => void; setQuery: (v: string) => void }) {
  ...
  async function approveSelected() {
    ...
    try {
      ...
      if (!result.ok) setMessage(result.error);
        ...
        setMessage("تم الاعتماد وإنشاء رابط التسجيل.");
      }
  ...
  async function copyRegistrationUrl() {
    ...
    try {
      await navigator.clipboard.writeText(absoluteRegistrationUrl);
      setCopied(true);
      window.setTimeout(() => setCopied(false), 1600);
    } catch {
      setMessage("تعذر نسخ الرابط تلقائيًا. حدده وانسخه يدويًا.");
    }
  }
