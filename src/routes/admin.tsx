  ...
  return <main className="mx-auto grid max-w-[1500px] gap-5 py-4 lg:py-8">
    ...
    <div className="grid gap-5 lg:grid-cols-[230px_minmax(0,1fr)]">
      ...
      <section className="min-w-0 grid gap-4">
        ...
        {tab === "orders" ? <Orders rows={orders} selected={selectedOrder} loading={ordersLoading} status={orderStatus} query={orderQuery} setStatus={setOrderStatus} setQuery={setOrderQuery} saving={saving} onStatus={changeOrderStatus} onArchive={archiveOrder} /> : null}
...
function Orders({ rows, selected, loading, status, query, setStatus, setQuery, saving, onStatus, onArchive }: { rows: PlatformOrder[]; selected: PlatformOrder | null; loading: boolean; status: OrderStatus | "all"; query: string; setStatus: (v: OrderStatus | "all") => void; setQuery: (v: string) => void; saving: string | null; onStatus: (id: string, status: OrderStatus) => Promise<void>; onArchive: (order: PlatformOrder) => Promise<void> }) {
  return <div className="grid gap-4 rounded-3xl border border-line bg-sand/20 p-3 md:p-4 lg:grid-cols-[1.05fr_.95fr]">
    <div className="grid content-start gap-3"><div className="grid gap-2 md:grid-cols-[1fr_190px]"><label className="relative"><Search className="pointer-events-none absolute start-3 top-1/2 size-4 -translate-y-1/2 text-muted" /><Input className="ps-9" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="اسم العميل أو الجوال أو المطعم أو رقم الطلب" /></label><select value={status} onChange={(e) => setStatus(e.target.value as OrderStatus | "all")} className="h-10 rounded-md border border-line bg-paper px-3 text-sm"><option value="all">كل الحالات</option>{Object.entries(ORDER_LABELS).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></div>
      {loading && !rows.length ? <Empty text="جار تحميل الطلبات…" /> : !rows.length ? <Empty text="لا توجد طلبات مطابقة." /> : <div className="grid gap-2">{rows.map((order) => <button key={order.id} type="button" onClick={() => undefined} className={cn("grid gap-3 rounded-2xl border bg-paper p-4 text-start", selected?.id === order.id ? "border-ink bg-sand/40" : "border-line hover:bg-sand/40")}><div className="flex items-start justify-between gap-3"><div><strong>#{order.orderNumber} · {order.customerName || "عميل"}</strong><p className="mt-1 text-xs text-muted">{order.restaurantName} · {order.branchName}</p></div><Badge status={order.status} /></div><div className="flex flex-wrap items-center justify-between gap-2 text-xs text-muted"><span>{order.itemCount} عناصر · {order.total.toFixed(2)} {order.currency}</span><span>{fmt(order.createdAt)}</span></div><div className="flex flex-wrap gap-2" onClick={(event) => event.stopPropagation()}>{order.customerPhone ? <a href={phoneHref(order.customerPhone)} className="inline-flex min-h-9 items-center gap-1.5 rounded-lg border border-line px-2.5 text-xs" aria-label={`الاتصال بالعميل ${order.customerName || ""}`}><Phone className="size-3.5" />اتصال</a> : null}{order.customerPhone ? <a href={whatsappHref(order.customerPhone)} target="_blank" rel="noreferrer" className="inline-flex min-h-9 items-center gap-1.5 rounded-lg border border-line px-2.5 text-xs" aria-label={`مراسلة العميل ${order.customerName || ""} عبر واتساب`}><MessageCircle className="size-3.5" />WhatsApp</a> : null}{order.customerEmail ? <a href={`mailto:${order.customerEmail}`} className="inline-flex min-h-9 items-center gap-1.5 rounded-lg border border-line px-2.5 text-xs" aria-label={`إرسال بريد إلى ${order.customerName || "العميل"}`}><Mail className="size-3.5" />بريد</a> : null}</div></button>)}</div>}
    </div>
