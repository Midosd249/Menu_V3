...
function PlatformAdminPage() {
  ...
  return <main className="mx-auto grid max-w-[1500px] gap-5 py-4 lg:py-8">
    <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
      ...
      <div className="flex flex-wrap gap-2"><Button variant="outline" onClick={() => void navigate({ to: "/admin/onboarding" })}>اعتماد العملاء الجدد</Button><Button variant="outline" onClick={() => { void load(); if (tab === "orders") void loadOrders(); }} disabled={loading || ordersLoading}><RefreshCw className={cn("size-4", (loading || ordersLoading) && "animate-spin")} /> تحديث البيانات</Button></div>
    </header>
...
function Overview({ platform, leads, onTab, onOpen }: { platform: PlatformDashboard; leads: AdminDashboard; onTab: (tab: Tab) => void; onOpen: () => void }) { return <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
  ...
  <LeadContactPanel leads={leads.leads} onOpen={onOpen} />
...
function Leads({ leads, status, query, setStatus, setQuery, onOpen }: { leads: AdminDashboard; status: LeadStatus | "all"; query: string; setStatus: (v: LeadStatus | "all") => void; setQuery: (v: string) => void; onOpen: () => void }) { return <div className="grid gap-4"><div className="flex flex-col gap-3 rounded-2xl border border-line bg-paper p-3 md:flex-row md:items-center md:justify-between"><div className="grid flex-1 gap-3 md:grid-cols-[1fr_220px]"><label className="relative"><Search className="absolute start-3 top-1/2 size-4 -translate-y-1/2 text-muted" /><Input className="ps-9" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="ابحث باسم النشاط أو المسؤول أو المدينة" /></label><select value={status} onChange={(e) => setStatus(e.target.value as LeadStatus | "all")} className="h-10 rounded-md border border-line bg-paper px-3 text-sm"><option value="all">كل الحالات</option>{LEAD_STATUSES.map((s) => <option key={s} value={s}>{LABELS[s]}</option>)}</select></div><Button type="button" onClick={onOpen}><ExternalLink className="size-4" />مركز اعتماد العملاء</Button></div><div className="grid gap-2">{leads.leads.map((l) => <article key={l.id} className="rounded-2xl border border-line bg-paper p-4"><div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between"><div><b>{l.businessName}</b><p className="text-sm text-muted">{l.contactName} · {l.city || "—"} · {l.contactPhone}</p></div><span className="rounded-full bg-sand px-2.5 py-1 text-xs">{LABELS[l.status]}</span></div><div className="mt-2 text-xs text-muted">{fmt(l.createdAt)}{l.details ? ` · ${l.details}` : ""}</div></article>)}{!leads.leads.length ? <Empty text="لا توجد نتائج." /> : null}</div></div>; }
...
function Subscriptions({ platform }: { platform: PlatformDashboard }) {
  const byPlan = platform.tenants.reduce<Record<string, number>>((acc, t) => {
    acc[t.planCode] = (acc[t.planCode] ?? 0) + 1;
    return acc;
  }, {});
  return <div className="grid gap-4 md:grid-cols-3">
    <article className="grid gap-3 rounded-2xl border border-line bg-paper p-5"><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-sand/60"><Wallet className="size-5" /></span><h2 className="font-semibold">نشطة</h2></div><p className="text-sm leading-6 text-muted">الاشتراكات النشطة ضمن المطاعم الحالية.</p><strong className="text-2xl">{platform.activeSubscriptionCount.toLocaleString("ar-SA")}</strong></article>
    <article className="grid gap-3 rounded-2xl border border-line bg-paper p-5"><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-sand/60"><Wallet className="size-5" /></span><h2 className="font-semibold">تجريبية</h2></div><p className="text-sm leading-6 text-muted">الاشتراكات التجريبية ضمن المطاعم الحالية.</p><strong className="text-2xl">{platform.trialSubscriptionCount.toLocaleString("ar-SA")}</strong></article>
