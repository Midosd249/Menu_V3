...
function Overview({ platform, leads, onTab }: { platform: PlatformDashboard; leads: AdminDashboard; onTab: (tab: Tab) => void }) { return <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
  ...
  <LeadContactPanel leads={leads.leads} />
...
function Leads({ leads, status, query, setStatus, setQuery }: { leads: AdminDashboard; status: LeadStatus | "all"; query: string; setStatus: (v: LeadStatus | "all") => void; setQuery: (v: string) => void }) { return <div className="grid gap-4"><div className="flex flex-col gap-3 rounded-2xl border border-line bg-paper p-3 md:flex-row md:items-center md:justify-between"><div className="grid flex-1 gap-3 md:grid-cols-[1fr_220px]"><label className="relative"><Search className="absolute start-3 top-1/2 size-4 -translate-y-1/2 text-muted" /><Input className="ps-9" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="ابحث باسم النشاط أو المسؤول أو المدينة" /></label><select value={status} onChange={(e) => setStatus(e.target.value as LeadStatus | "all")} className="h-10 rounded-md border border-line bg-paper px-3 text-sm"><option value="all">كل الحالات</option>{LEAD_STATUSES.map((s) => <option key={s} value={s}>{LABELS[s]}</option>)}</select></div><a href="/admin/onboarding" className="inline-flex min-h-10 items-center justify-center gap-2 rounded-xl bg-ink px-4 py-2 text-sm font-medium text-paper"><ExternalLink className="size-4" />مركز اعتماد العملاء</a></div><div className="grid gap-2">{leads.leads.map((l) => <article key={l.id} className="rounded-2xl border border-line bg-paper p-4"><div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between"><div><b>{l.businessName}</b><p className="text-sm text-muted">{l.contactName} · {l.city || "—"} · {l.contactPhone}</p></div><span className="rounded-full bg-sand px-2.5 py-1 text-xs">{LABELS[l.status]}</span></div><div className="mt-2 text-xs text-muted">{fmt(l.createdAt)}{l.details ? ` · ${l.details}` : ""}</div></article>)}{!leads.leads.length ? <Empty text="لا توجد نتائج." /> : null}</div></div>; }
