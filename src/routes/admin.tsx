...
function PlatformAdminPage() {
  ...
  return <main className="mx-auto grid max-w-[1500px] gap-5 py-4 lg:py-8">
    <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
      ...
      <div className="flex flex-wrap gap-2"><Button variant="outline" onClick={() => window.location.assign("/admin/onboarding")}>اعتماد العملاء الجدد</Button><Button variant="outline" onClick={() => { void load(); if (tab === "orders") void loadOrders(); }} disabled={loading || ordersLoading}><RefreshCw className={cn("size-4", (loading || ordersLoading) && "animate-spin")} /> تحديث البيانات</Button></div>
    </header>
...
function Overview({ platform, leads, onTab }: { platform: PlatformDashboard; leads: AdminDashboard; onTab: (tab: Tab) => void }) { return <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
  ...
  <LeadContactPanel leads={leads.leads} onOpen={() => window.location.assign("/admin/onboarding")} />
