...
import { getAdminDashboard, LEAD_STATUSES, updateLead, type AdminDashboard, type LeadStatus } from "@/lib/menu/admin";
...
const ADMIN_GROUPS: Array<{ id: string; label: string; items: Tab[] }> = [
  { id: "overview", label: "نظرة عامة", items: ["overview"] },
  { id: "customers", label: "العملاء", items: ["tenants", "clients", "branches"] },
  { id: "commerce", label: "التجارة والتشغيل", items: ["orders", "subscriptions", "requests"] },
  { id: "sales", label: "المبيعات", items: ["leads", "projects"] },
  { id: "intelligence", label: "الذكاء التشغيلي", items: ["analytics", "activity"] },
  ...
  return <main className="mx-auto grid max-w-[1500px] gap-5 py-4 lg:py-8">
    ...
    <div className="grid gap-5 lg:grid-cols-[250px_minmax(0,1fr)]">
      ...
      <section className="min-w-0 grid gap-4">
        ...
        {loading && !platform.tenants.length && !leads.leads.length ? <LoadingState label="جارٍ تحميل مساحة إدارة المنصة…" /> : null}
        {!loading || platform.tenants.length || leads.leads.length ? <>
          <section className="rounded-2xl border border-line bg-paper p-4">
            ...
            <div className="mt-2 grid gap-x-6 md:grid-cols-2 xl:grid-cols-4">
              ...
              <MetricRow label="Leads جديدة" value={platform.newLeadCount.toLocaleString("ar-SA")} />
            </div>
          </section>
...
function AdminNavigation({ tab, platform, onSelect }: { tab: Tab; platform: PlatformDashboard; onSelect: (tab: Tab) => void }) {
  return <aside className="h-fit rounded-2xl border border-line bg-paper p-2 lg:sticky lg:top-4" aria-label="تنقل إدارة المنصة">
    <div className="px-3 py-3"><p className="text-xs font-semibold text-muted">Platform Admin</p><p className="mt-1 text-xs leading-5 text-muted">مساحات تشغيل المنصة الحالية</p></div>
    <nav className="grid gap-2">
      {ADMIN_GROUPS.map((group) => <section key={group.id} className="rounded-xl border border-line/70 bg-sand/20 p-1.5" aria-labelledby={`admin-group-${group.id}`}>
        <h2 id={`admin-group-${group.id}`} className="px-2.5 py-2 text-[11px] font-semibold tracking-wide text-muted">{group.label}</h2>
        <div className="grid gap-1">
          {group.items.map((id) => {
            const item = NAV.find((candidate) => candidate.id === id);
            if (!item) return null;
            const Icon = item.icon;
            const badge = id === "orders" && platform.openOrderCount > 0 ? platform.openOrderCount : null;
            return <button key={item.id} type="button" aria-current={tab === item.id ? "page" : undefined} onClick={() => onSelect(item.id)} className={cn("flex min-h-11 w-full items-center gap-3 rounded-lg px-3 text-sm text-start transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring", tab === item.id ? "bg-ink text-paper" : "text-ink-soft hover:bg-sand/60 hover:text-ink")}><Icon className="size-4 shrink-0" aria-hidden /> <span className="min-w-0 flex-1 truncate">{item.label}</span>{badge ? <span className={cn("rounded-full px-2 py-0.5 text-[11px]", tab === item.id ? "bg-paper text-ink" : "bg-sand text-ink")}>{badge}</span> : null}</button>;
          })}
        </div>
      </section>)}
    </nav>
...
function LeadContactPanel({ leads, onApproval }: { leads: AdminDashboard["leads"]; onApproval: () => void }) {
  ...
  return <article className="grid gap-4 rounded-2xl border border-line bg-paper p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-sand/60"><Phone className="size-5" /></span><h2 className="font-semibold">التواصل واعتماد الطلبات</h2></div><p className="mt-2 text-sm text-muted">طلبات الموقع الواردة، مع وسائل الاتصال المباشر والتحكم في حالة الطلب واعتماد العميل وإنشاء رابط التسجيل.</p></div><button type="button" onClick={onApproval} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-xl bg-ink px-4 py-2 text-sm font-medium text-paper"><ExternalLink className="size-4" />فتح مركز الاعتماد</button></div><div className="grid gap-3 md:grid-cols-3">{visible.map((lead) => <div key={lead.id} className="grid gap-3 rounded-xl border border-line bg-sand/20 p-4"><div className="min-w-0"><b className="block truncate">{lead.businessName}</b><p className="mt-1 truncate text-xs text-muted">{lead.contactName} · {lead.city || "—"}</p><p className="mt-1 text-xs text-muted">{LABELS[lead.status]}</p></div><div className="flex flex-wrap gap-2">{lead.contactPhone ? <a href={phoneHref(lead.contactPhone)} className="inline-flex min-h-9 items-center gap-1.5 rounded-lg border border-line px-2.5 text-xs"><Phone className="size-3.5" />اتصال</a> : null}{lead.contactPhone ? <a href={whatsappHref(lead.contactPhone)} target="_blank" rel="noreferrer" className="inline-flex min-h-9 items-center gap-1.5 rounded-lg border border-line px-2.5 text-xs"><MessageCircle className="size-3.5" />WhatsApp</a> : null}{lead.contactEmail ? <a href={`mailto:${lead.contactEmail}`} className="inline-flex min-h-9 items-center gap-1.5 rounded-lg border border-line px-2.5 text-xs"><Mail className="size-3.5" />بريد</a> : null}</div></div>)}{!visible.length ? <div className="md:col-span-3 rounded-xl border border-dashed border-line p-6 text-center text-sm text-muted">لا توجد طلبات عملاء حالياً.</div> : null}</div></article>;
}
...
function Orders({ rows, selected, loading, status, query, setStatus, setQuery, saving, onStatus, onArchive, onSelect }: { rows: PlatformOrder[]; selected: PlatformOrder | null; loading: boolean; status: OrderStatus | "all"; query: string; setStatus: (v: OrderStatus | "all") => void; setQuery: (v: string) => void; saving: string | null; onStatus: (id: string, status: OrderStatus) => Promise<void>; onArchive: (order: PlatformOrder) => Promise<void>; onSelect: (order: PlatformOrder) => void }) {
  return <div className="grid gap-4 rounded-3xl border border-line bg-sand/20 p-3 md:p-4 lg:grid-cols-[1.05fr_.95fr]"><div className="grid content-start gap-3"><div className="grid gap-2 md:grid-cols-[1fr_190px]"><label className="relative"><Search className="pointer-events-none absolute start-3 top-1/2 size-4 -translate-y-1/2 text-muted" /><Input className="ps-9" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="اسم العميل أو الجوال أو المطعم أو رقم الطلب" /></label><select value={status} onChange={(e) => setStatus(e.target.value as OrderStatus | "all")} className="h-10 rounded-md border border-line bg-paper px-3 text-sm"><option value="all">كل الحالات</option>{Object.entries(ORDER_LABELS).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></div>{loading && !rows.length ? <Empty text="جار تحميل الطلبات…" /> : !rows.length ? <Empty text="لا توجد طلبات مطابقة." /> : <div className="grid gap-2">{rows.map((order) => <article key={order.id} className={cn("grid gap-3 rounded-2xl border bg-paper p-4", selected?.id === order.id ? "border-ink bg-sand/40" : "border-line")}><button type="button" onClick={() => onSelect(order)} className="grid gap-2 text-start"><div className="flex items-start justify-between gap-3"><div><strong>#{order.orderNumber} · {order.customerName || "عميل"}</strong><p className="mt-1 text-xs text-muted">{order.restaurantName} · {order.branchName}</p></div><Badge status={order.status} /></div><div className="flex flex-wrap items-center justify-between gap-2 text-xs text-muted"><span>{order.itemCount} عناصر · {order.total.toFixed(2)} {order.currency}</span><span>{fmt(order.createdAt)}</span></div></button><div className="flex flex-wrap gap-2">{order.customerPhone ? <a href={phoneHref(order.customerPhone)} className="inline-flex min-h-9 items-center gap-1.5 rounded-lg border border-line px-2.5 text-xs" aria-label={`الاتصال بالعميل ${order.customerName || ""}`}><Phone className="size-3.5" />اتصال</a> : null}{order.customerPhone ? <a href={whatsappHref(order.customerPhone)} target="_blank" rel="noreferrer" className="inline-flex min-h-9 items-center gap-1.5 rounded-lg border border-line px-2.5 text-xs" aria-label={`مراسلة العميل ${order.customerName || ""} عبر واتساب`}><MessageCircle className="size-3.5" />WhatsApp</a> : null}{order.customerEmail ? <a href={`mailto:${order.customerEmail}`} className="inline-flex min-h-9 items-center gap-1.5 rounded-lg border border-line px-2.5 text-xs" aria-label={`إرسال بريد إلى ${order.customerName || "العميل"}`}><Mail className="size-3.5" />بريد</a> : null}</div></article>)}</div>}</div><OrderDetail order={selected} saving={saving} onStatus={onStatus} onArchive={onArchive} /></div>;
}
...
function OrderDetail({ order, saving, onStatus, onArchive }: { order: PlatformOrder | null; saving: string | null; onStatus: (id: string, status: OrderStatus) => Promise<void>; onArchive: (order: PlatformOrder) => Promise<void> }) {
  ...
  return <aside className="grid content-start gap-5 rounded-2xl border border-line bg-paper p-5 lg:sticky lg:top-5 lg:h-fit"><div className="flex items-start justify-between gap-3"><div><p className="text-xs text-muted">طلب #{order.orderNumber}</p><h2 className="mt-1 text-2xl font-semibold">{order.customerName || "عميل"}</h2><p className="mt-1 text-sm text-muted">{order.restaurantName} · {order.branchName}</p></div><Badge status={order.status} /></div><div className="flex flex-wrap gap-2">{order.customerPhone ? <a href={phoneHref(order.customerPhone)} className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-line px-3 text-sm"><Phone className="size-4" />اتصال</a> : null}{order.customerPhone ? <a href={whatsappHref(order.customerPhone)} target="_blank" rel="noreferrer" className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-line px-3 text-sm"><MessageCircle className="size-4" />WhatsApp</a> : null}{order.customerEmail ? <a href={`mailto:${order.customerEmail}`} className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-line px-3 text-sm"><Mail className="size-4" />البريد</a> : null}</div><div className="grid gap-2 text-sm"><p><span className="text-muted">الجوال:</span> {order.customerPhone || "—"}</p>{order.customerEmail ? <p className="break-all"><span className="text-muted">البريد:</span> {order.customerEmail}</p> : null}<p><span className="text-muted">المصدر:</span> {order.source}</p><p><span className="text-muted">الإجمالي:</span> {order.total.toFixed(2)} {order.currency}</p>{order.notes ? <p><span className="text-muted">ملاحظات الطلب:</span> {order.notes}</p> : null}</div><div className="grid gap-2">{order.items.map((item) => <div key={item.id} className="grid gap-1 rounded-xl bg-sand/40 px-3 py-2 text-sm"><div className="flex justify-between gap-3"><span>{item.quantity} × {item.productNameAr}</span><span>{item.lineTotal.toFixed(2)}</span></div>{item.selectedOptions.length ? <p className="text-xs text-muted">{item.selectedOptions.map((option) => option.type === "note" ? `ملاحظة: ${option.nameAr}` : option.nameAr).join("، ")}</p> : null}</div>)}</div><div className="grid grid-cols-2 gap-2">{(["new", "confirmed", "preparing", "ready", "completed", "cancelled"] as OrderStatus[]).map((next) => <Button key={next} variant={next === order.status ? "default" : "outline"} disabled={currentSaving || next === order.status} onClick={() => void onStatus(order.id, next)}>{next === "completed" ? <CheckCircle2 className="size-4" /> : next === "cancelled" ? <XCircle className="size-4" /> : next === "ready" ? <PackageCheck className="size-4" /> : next === "preparing" ? <Clock3 className="size-4" /> : null}{ORDER_LABELS[next]}</Button>)}</div><Button variant="outline" disabled={currentSaving} onClick={() => void onArchive(order)}><Archive className="size-4" />إزالة من لوحة التشغيل</Button><p className="text-xs leading-5 text-muted">الإزالة هنا أرشفة آمنة وليست حذفًا نهائيًا؛ يحافظ النظام على سجل الطلب وعناصره وتاريخ حالاته.</p></aside>;
}
...
function Leads({ leads, status, query, setStatus, setQuery }: { leads: AdminDashboard; status: LeadStatus | "all"; query: string; setStatus: (v: LeadStatus | "all") => void; setQuery: (v: string) => void }) {
  const [selectedId, setSelectedId] = useState<string | null>(leads.leads[0]?.id ?? null);
  const [notes, setNotes] = useState("");
  const [saving, setSaving] = useState(false);
  const [message, setMessage] = useState("");
  const [registrationUrl, setRegistrationUrl] = useState("");
  const [copied, setCopied] = useState(false);
  const selected = leads.leads.find((lead) => lead.id === selectedId) ?? leads.leads[0] ?? null;
  const absoluteRegistrationUrl = registrationUrl ? `${window.location.origin}${registrationUrl}` : "";
  useEffect(() => { setSelectedId((current) => current && leads.leads.some((lead) => lead.id === current) ? current : leads.leads[0]?.id ?? null); }, [leads.leads]);
  useEffect(() => { setNotes(selected?.notes ?? ""); setMessage(""); setRegistrationUrl(""); setCopied(false); }, [selected?.id]);
  async function setLeadStatus(nextStatus: LeadStatus) {
    ...
    try {
      ...
      else setMessage(nextStatus === "lost" ? "تم رفض الطلب." : "تم تحديث حالة الطلب.");
    } catch (error) { setMessage(error instanceof Error ? error.message : "تعذر تحديث الطلب"); }
    finally { setSaving(false); }
  }
  async function approveSelected() {
    if (!selected || saving) return;
    setSaving(true); setMessage(""); setRegistrationUrl(""); setCopied(false);
    try {
      const result = await approveLead({ data: { leadId: selected.id } });
      if (!result.ok) setMessage(result.error);
      else { setRegistrationUrl(result.data.registrationUrl); setMessage("تم الاعتماد وإنشاء رابط التسجيل."); }
    } catch (error) { setMessage(error instanceof Error ? error.message : "تعذر اعتماد العميل"); }
    finally { setSaving(false); }
  }
  async function copyRegistrationUrl() {
    if (!absoluteRegistrationUrl) return;
    try { await navigator.clipboard.writeText(absoluteRegistrationUrl); setCopied(true); window.setTimeout(() => setCopied(false), 1600); }
    catch { setMessage("تعذر نسخ الرابط تلقائيًا. حدده وانسخه يدويًا."); }
  }
  return <div className="grid gap-4"><div className="flex flex-col gap-3 rounded-2xl border border-line bg-paper p-3 md:flex-row md:items-center md:justify-between"><div className="grid flex-1 gap-3 md:grid-cols-[1fr_220px]"><label className="relative"><Search className="absolute start-3 top-1/2 size-4 -translate-y-1/2 text-muted" /><Input className="ps-9" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="ابحث باسم النشاط أو المسؤول أو المدينة" /></label><select value={status} onChange={(e) => setStatus(e.target.value as LeadStatus | "all")} className="h-10 rounded-md border border-line bg-paper px-3 text-sm"><option value="all">كل الحالات</option>{LEAD_STATUSES.map((s) => <option key={s} value={s}>{LABELS[s]}</option>)}</select></div></div><div className="grid gap-4 lg:grid-cols-[1fr_1.05fr]"><div className="grid gap-2">{leads.leads.map((l) => <button key={l.id} type="button" onClick={() => setSelectedId(l.id)} className={cn("grid gap-2 rounded-2xl border p-4 text-start", selected?.id === l.id ? "border-ink bg-sand/30" : "border-line bg-paper")}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><b className="block truncate">{l.businessName}</b><p className="mt-1 text-sm text-muted">{l.contactName} · {l.city || "—"}</p><p className="mt-1 text-xs text-muted">{l.contactPhone || l.contactEmail || "—"}</p></div><span className="rounded-full bg-sand px-2.5 py-1 text-xs">{LABELS[l.status]}</span></div></button>)}{!leads.leads.length ? <Empty text="لا توجد نتائج." /> : null}</div>{selected ? <aside className="grid content-start gap-4 rounded-2xl border border-line bg-paper p-5"><div><p className="text-xs text-muted">تفاصيل الطلب</p><h2 className="mt-1 text-2xl font-semibold">{selected.businessName}</h2><p className="mt-1 text-sm text-muted">{selected.contactName}</p></div><div className="grid gap-2 rounded-xl border border-line bg-sand/20 p-4 text-sm"><Info label="المطعم" value={selected.businessName} /><Info label="المدينة" value={selected.city || "—"} /><Info label="اسم جهة الاتصال" value={selected.contactName} /><Info label="رقم الجوال" value={selected.contactPhone || "—"} dir="ltr" /><Info label="البريد" value={selected.contactEmail || "—"} dir="ltr" /><Info label="الحالة" value={LABELS[selected.status]} /><Info label="تاريخ الطلب" value={fmt(selected.createdAt)} /></div><div className="flex flex-wrap gap-2">{selected.contactPhone ? <a href={phoneHref(selected.contactPhone)} className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-line px-3 text-sm"><Phone className="size-4" />اتصال</a> : null}{selected.contactPhone ? <a href={whatsappHref(selected.contactPhone)} target="_blank" rel="noreferrer" className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-line px-3 text-sm"><MessageCircle className="size-4" />WhatsApp</a> : null}{selected.contactEmail ? <a href={`mailto:${selected.contactEmail}`} className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-line px-3 text-sm"><Mail className="size-4" />بريد</a> : null}</div><label className="grid gap-2 text-sm"><span className="font-medium">ملاحظات الإدارة</span><textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="ملاحظة داخلية…" className="min-h-24 rounded-xl border border-line bg-paper p-3 outline-none focus:ring-2 focus:ring-ink/20" /></label>{registrationUrl ? <div className="grid gap-3 rounded-2xl border border-ink/20 bg-sand/40 p-4" role="status"><div><p className="font-semibold">تم إنشاء رابط التسجيل</p><p className="mt-1 text-xs leading-5 text-muted">أرسله للعميل ليكمل إنشاء حسابه ومساحة المطعم. الرابط صالح لمدة 7 أيام ويُستخدم مرة واحدة.</p></div><div className="flex flex-col gap-2 sm:flex-row"><input readOnly value={absoluteRegistrationUrl} dir="ltr" aria-label="رابط تسجيل العميل" className="min-h-11 min-w-0 flex-1 rounded-xl border border-line bg-paper px-3 text-sm" /><Button type="button" variant="outline" onClick={() => void copyRegistrationUrl()}><Copy className="size-4" />{copied ? "تم النسخ" : "نسخ الرابط"}</Button><a href={absoluteRegistrationUrl} target="_blank" rel="noreferrer" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-line px-3 text-sm"><ExternalLink className="size-4" />فتح الرابط</a></div></div> : null}{message ? <div className="rounded-xl border border-line bg-sand/40 px-3 py-2 text-sm break-words" role="status">{message}</div> : null}<div className="grid gap-2 sm:grid-cols-2"><Button disabled={saving} onClick={() => void approveSelected()}><UserCheck className="size-4" />اعتماد وإنشاء رابط التسجيل</Button><Button variant="outline" disabled={saving} onClick={() => void setLeadStatus("contacted")}><Phone className="size-4" />تم التواصل</Button><Button variant="outline" disabled={saving} onClick={() => void setLeadStatus("lost")}><XCircle className="size-4" />رفض الطلب</Button><Button variant="outline" disabled={saving} onClick={() => void setLeadStatus(selected.status)}><CheckCircle2 className="size-4" />حفظ الملاحظات</Button></div></aside> : <Empty text="اختر طلبًا لعرض أدوات التحكم." />}</div></div>;
}
function Info({ label, value, dir }: { label: string; value: string; dir?: "ltr" | "rtl" }) { return <div className="flex items-start justify-between gap-4 border-b border-line/60 pb-2 last:border-b-0 last:pb-0"><span className="text-muted">{label}</span><strong dir={dir} className="text-end break-all">{value}</strong></div>; }
...
function Subscriptions({ platform }: { platform: PlatformDashboard }) { const byPlan = platform.tenants.reduce<Record<string, number>>((acc, t) => { acc[t.planCode] = (acc[t.planCode] ?? 0) + 1; return acc; }, {}); return <div className="grid gap-4 md:grid-cols-3"><article className="grid gap-3 rounded-2xl border border-line bg-paper p-5"><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-sand/60"><Wallet className="size-5" /></span><h2 className="font-semibold">نشطة</h2></div><p className="text-sm leading-6 text-muted">الاشتراكات النشطة ضمن المطاعم الحالية.</p><strong className="text-2xl">{platform.activeSubscriptionCount.toLocaleString("ar-SA")}</strong></article><article className="grid gap-3 rounded-2xl border border-line bg-paper p-5"><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-sand/60"><Wallet className="size-5" /></span><h2 className="font-semibold">تجريبية</h2></div><p className="text-sm leading-6 text-muted">الاشتراكات التجريبية ضمن المطاعم الحالية.</p><strong className="text-2xl">{platform.trialSubscriptionCount.toLocaleString("ar-SA")}</strong></article><article className="rounded-2xl border border-line bg-paper p-5"><h2 className="font-semibold">توزيع الخطط</h2><div className="mt-4 grid gap-2">{Object.entries(byPlan).map(([plan, count]) => <div key={plan} className="flex justify-between rounded-xl border border-line px-3 py-2 text-sm"><span>{plan}</span><b>{count}</b></div>)}{!Object.keys(byPlan).length ? <p className="text-sm text-muted">لا توجد بيانات خطط.</p> : null}</div></article></div>; }
function Analytics({ platform }: { platform: PlatformDashboard }) { const a = platform.analytics; return <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3"><MetricRow label="زيارات المنيو" value={a.visits.toLocaleString("ar-SA")} /><MetricRow label="مشاهدات الأصناف" value={a.productViews.toLocaleString("ar-SA")} /><MetricRow label="مسح QR" value={a.qrScans.toLocaleString("ar-SA")} /><MetricRow label="نقرات WhatsApp" value={a.whatsappClicks.toLocaleString("ar-SA")} /><MetricRow label="كل الطلبات" value={a.orders.toLocaleString("ar-SA")} /><MetricRow label="الطلبات المكتملة" value={a.completedOrders.toLocaleString("ar-SA")} /></div>; }
