...
function OrderDetail({ order, onStatus }: { order: AdminOrder | null; onStatus: (id: string, status: OrderStatus) => Promise<void> }) {
  if (!order) return <Empty text="اختر طلبًا لعرض تفاصيله." />;
  return <aside className="grid content-start gap-5 rounded-2xl border border-line bg-paper p-5 lg:sticky lg:top-5 lg:h-fit">
    <div><p className="text-xs text-muted">طلب #{order.orderNumber}</p><h2 className="mt-1 text-2xl font-semibold">{order.customerName || "عميل"}</h2><p className="mt-1 text-sm text-muted">{order.branchName}</p></div>
    <div className="grid gap-2 text-sm"><p><span className="text-muted">الجوال:</span> {order.customerPhone || "—"}</p>{order.customerEmail ? <p><span className="text-muted">البريد:</span> {order.customerEmail}</p> : null}<p><span className="text-muted">الإجمالي:</span> {order.total.toFixed(2)} {order.currency}</p>{order.notes ? <p><span className="text-muted">ملاحظات الطلب:</span> {order.notes}</p> : null}</div>
    <div className="grid gap-2">{order.items.map((item) => { const itemNotes = item.selectedOptions.filter((option) => option.type === "note"); const selections = item.selectedOptions.filter((option) => option.type !== "note"); return <div key={item.id} className="grid gap-1 rounded-xl bg-sand/40 px-3 py-2 text-sm"><div className="flex justify-between gap-3"><span>{item.quantity} × {item.productNameAr}</span><span>{item.lineTotal.toFixed(2)}</span></div>{selections.length ? <p className="text-xs text-muted">{selections.map((option) => option.nameAr).join("، ")}</p> : null}{itemNotes.map((note) => <p key={`${item.id}-${note.id}`} className="rounded-lg bg-paper/70 px-2 py-1 text-xs leading-5"><strong>ملاحظة الصنف:</strong> {note.nameAr}</p>)}</div>; })}</div>
    <div className="grid grid-cols-2 gap-2">{ORDER_STATUSES.filter((s) => s !== "cancelled" || order.status !== "completed").map((next) => <Button key={next} variant={next === order.status ? "default" : "outline"} disabled={next === order.status} onClick={() => void onStatus(order.id, next)}>{next === "cancelled" ? <XCircle className="size-4" /> : null}{labels[next]}</Button>)}</div>
  </aside>;
}
