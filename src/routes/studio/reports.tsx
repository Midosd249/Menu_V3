...
function ReportContent({ report, lang, text, onPrint, onWhatsApp, onRefresh, refreshing, waState }: { report: MenuReport; lang: "ar" | "en"; text: string; onPrint: () => void; onWhatsApp: () => void; onRefresh: () => void; refreshing: boolean; waState: { status: "idle" | "loading" | "ready" | "error"; message?: string } }) {
  ...
  return <>
    ...
    <section className="grid gap-3 rounded-3xl border border-line p-6">
      ...
      <div className="grid gap-2 text-sm sm:grid-cols-2 lg:grid-cols-4"><Finding label={lang === "ar" ? "صور ناقصة" : "Missing images"} value={report.missingImages} /><Finding label={lang === "ar" ? "وصف عربي ناقص" : "Missing Arabic descriptions"} value={report.missingDescriptions} /><Finding label={lang === "ar" ? "إنجليزية ناقصة" : "Missing English content"} value={report.missingEnglish} /><Finding label={lang === "ar" ? "أصناف غير متاحة" : "Unavailable products"} value={report.unavailableProducts} /></div>
    </section>
