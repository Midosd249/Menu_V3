...
const publicMenuSearchSchema = z.object({ branch: z.string().max(63).optional(), lang: z.enum(["ar", "en"]).optional() });
type PublicMenuRouteData = { menu: PublicMenu; locale: Lang; localeAvailable: boolean };
...
export const Route = createFileRoute("/m/$slug")({
  ...
  loader: async ({ params, deps }) => {
    ...
    return { ok: true as const, data: { menu: result.data, locale, localeAvailable: locale === requestedLocale } };
  },
  head: ({ loaderData, params, matches }) => {
    ...
    if (loaderData?.ok) {
      ...
      return { meta: [{ title: seo.title }, { name: "description", content: seo.description }, { name: "robots", content: localeAvailable ? "index, follow" : "noindex, follow" }, { property: "og:type", content: "website" }, { property: "og:title", content: seo.title }, { property: "og:description", content: seo.description }, { property: "og:locale", content: locale === "ar" ? "ar_SA" : "en_US" }, ...(seo.image ? [{ property: "og:image", content: seo.image }] : [])], links: hasBranchChild ? [] : [{ rel: "canonical", href: seo.canonical }, ...seo.alternates.map((alternate) => ({ rel: "alternate", hreflang: alternate.hreflang, href: alternate.href }))], scripts: hasBranchChild ? [] : [{ type: "application/ld+json", children: JSON.stringify(seo.schema) }] };
    }
    return { meta: [{ title: "المنيو غير موجود" }, { name: "robots", content: "noindex, nofollow" }], links: hasBranchChild ? [] : [{ rel: "canonical", href: pathname }] };
  },
...
function PublicMenuPage() { const { slug } = Route.useParams(); const { branch, lang } = Route.useSearch(); const loaderData = Route.useLoaderData(); const menuData = loaderData?.ok ? loaderData.data as PublicMenuRouteData : undefined; return <MenuLoader slug={slug} branch={branch} locale={menuData?.locale ?? lang ?? "ar"} initialMenu={menuData?.menu} />; }
