...
import { getThemeFamily, normalizeThemeKey } from "@/lib/theme";
...
export function MenuLoader({ slug, branch, locale, initialMenu, previewTheme }: { slug: string; branch?: string; locale: Lang; initialMenu?: PublicMenu; previewTheme?: ThemeKey }) {
  ...
  if (state.status === "loading") return <LoadingState label="جارٍ تحميل المنيو…" />; if (state.status === "error") return <ErrorState message={state.message} onRetry={state.retry} />; const activeTheme = previewTheme ?? state.menu.tenant.themeKey; const family = getThemeFamily(activeTheme); return <div className="menu-public-shell"><MenuThemeController theme={activeTheme} preview={Boolean(previewTheme)} />{family === "specialty-cafe" ? <SpecialtyCafeTemplate menu={{ ...state.menu, tenant: { ...state.menu.tenant, themeKey: activeTheme } }} /> : family === "bakery-dessert" ? <BakeryDessertTemplate menu={{ ...state.menu, tenant: { ...state.menu.tenant, themeKey: activeTheme } }} /> : family === "fast-casual" ? <FastCasualTemplate menu={{ ...state.menu, tenant: { ...state.menu.tenant, themeKey: activeTheme } }} /> : family === "fine-dining-hospitality" ? <FineDiningHospitalityTemplate menu={{ ...state.menu, tenant: { ...state.menu.tenant, themeKey: activeTheme } }} /> : family === "small-menu" ? <SmallMenuTemplate menu={{ ...state.menu, tenant: { ...state.menu.tenant, themeKey: activeTheme } }} /> : family === "contemporary-restaurant" ? <ContemporaryRestaurantTemplate menu={{ ...state.menu, tenant: { ...state.menu.tenant, themeKey: activeTheme } }} /> : <PublicMenuView menu={{ ...state.menu, tenant: { ...state.menu.tenant, themeKey: activeTheme } }} preview={Boolean(previewTheme)} />}</div>;
}
