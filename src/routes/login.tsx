  ...
  return (
    <main className="grid min-h-dvh place-items-center bg-paper px-5 py-10 text-ink">
      <div className="w-full max-w-md grid gap-6">
        ...
        {authEnabled ? <>
          <div className="grid gap-2">{GROK_PROVIDERS.map((p) => <Button key={p.providerId} type="button" variant="outline" disabled={busy} onClick={() => signIn(p.providerId, { callbackURL: invite ? `/invite/${encodeURIComponent(invite)}` : "/studio" })}>{p.providerId === "google" ? t(copy.auth.google, lang) : t(copy.auth.x, lang)}</Button>)}</div>
        ...
        <section className="grid gap-3 border-t border-line pt-5" aria-label={lang === "ar" ? "مساحات العمل" : "Workspaces"}><p className="text-xs font-medium uppercase tracking-[0.16em] text-muted">{lang === "ar" ? "مساحات العمل" : "Workspaces"}</p><div className="grid gap-2 sm:grid-cols-2"><Link to="/studio" className="rounded-xl border border-line p-4 text-sm hover:bg-sand"><span className="font-semibold">Studio</span><span className="mt-1 block text-xs leading-5 text-muted">{lang === "ar" ? "إدارة المنيو والمنتجات والتصميم والفروع" : "Menu, products, design and branches"}</span></Link><Link to="/owner" className="rounded-xl border border-line p-4 text-sm hover:bg-sand"><span className="font-semibold">Owner</span><span className="mt-1 block text-xs leading-5 text-muted">{lang === "ar" ? "مركز الطلبات والعملاء المحتملين" : "Orders and lead center"}</span></Link></div></section>
