...
import { ArrowUpLeft, BarChart3, Check, ExternalLink, GitBranch, Globe2, Layers3, Menu, QrCode, ScanLine, Sparkles, Smartphone, Store, X } from "lucide-react";
...
const FEATURES = [
  ...
  { icon: GitBranch, ar: "إدارة فروع", en: "Branch management", bodyAr: "إدارة الفروع ضمن حدود الباقة وبسياق واضح.", bodyEn: "Branch-aware management within your plan limits." },
...
const FAQS = [
  ...
  { qAr: "هل أستطيع إدارة أكثر من فرع؟", qEn: "Can I manage multiple branches?", aAr: "نعم، ضمن حدود الباقة المختارة.", aEn: "Yes, within the selected plan limits." },
  { qAr: "هل يمكن تحديث الأسعار فوراً؟", qEn: "Can I update prices immediately?", aAr: "نعم، بعد نشر التعديل عبر مسار المحتوى العام في المنصة.", aEn: "Yes, after publishing through the platform's public-content path." },
  { qAr: "هل يوجد رابط عام للمنيو؟", qEn: "Is there a public menu link?", aAr: "نعم، ويمكن استخدامه مع QR والمشاركة المباشرة.", aEn: "Yes, and it can be used with QR and direct sharing." },
] as const;
...
function Home() {
  ...
  useEffect(() => {
    ...
    const theme = normalizeThemeKey(params.get("theme"));
    const validPlan = plan && COMMERCIAL_PLANS.some((item) => item.code === plan) ? plan : "";
    if (validPlan) setSelectedPlan(validPlan);
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      <header className="sticky top-0 z-50 border-b bg-background/95 backdrop-blur"><div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6"><Link to="/" className="font-semibold tracking-tight">Menu V3</Link><nav className="hidden items-center gap-6 md:flex">{navItems.map(([href, label]) => <a key={href} href={href} className="text-sm text-muted-foreground transition hover:text-foreground">{label}</a>)}</nav><div className="flex items-center gap-2"><LangToggle /><SignedOut><Button asChild size="sm"><Link to="/auth/sign-in">{lang === "ar" ? "دخول" : "Sign in"}</Link></Button></SignedOut><SignedIn><Button asChild size="sm"><Link to="/studio">{lang === "ar" ? "الاستوديو" : "Studio"}</Link></Button></SignedIn><button type="button" aria-label={lang === "ar" ? "فتح القائمة" : "Open menu"} className="rounded-md p-2 md:hidden" onClick={() => setMobileOpen((value) => !value)}>{mobileOpen ? <X size={20} /> : <Menu size={20} />}</button></div></div>{mobileOpen && <nav className="border-t px-4 py-3 md:hidden">{navItems.map(([href, label]) => <a key={href} href={href} onClick={() => setMobileOpen(false)} className="block py-2 text-sm">{label}</a>)}</nav>}</header>
      <section className="relative overflow-hidden px-4 pb-20 pt-16 sm:px-6 sm:pt-24"><div className="mx-auto grid max-w-7xl items-center gap-12 lg:grid-cols-[1.05fr_.95fr]"><div className="max-w-3xl"><div className="mb-5 inline-flex items-center gap-2 rounded-full border px-3 py-1 text-xs font-medium"><Sparkles size={14} />{lang === "ar" ? "منيو رقمي عربي أولاً" : "Arabic-first digital menus"}</div><h1 className="text-balance text-5xl font-semibold tracking-tight sm:text-6xl lg:text-7xl">{lang === "ar" ? "حوّل منيو مطعمك إلى تجربة تستحق الزيارة." : "Turn your restaurant menu into an experience worth visiting."}</h1><p className="mt-6 max-w-2xl text-lg leading-8 text-muted-foreground">{lang === "ar" ? "Menu V3 يمنح المطاعم والكافيهات منيو سريعاً، أنيقاً، ثنائي اللغة، ومصمماً للجوال من أول لمسة." : "Menu V3 gives restaurants and cafés a fast, elegant, bilingual menu designed mobile-first from the first tap."}</p><div className="mt-8 flex flex-col gap-3 sm:flex-row"><Button size="lg" onClick={() => choose()}>{lang === "ar" ? "ابدأ طلبك" : "Start your request"}<ArrowUpLeft className="ms-2" size={18} /></Button><Button asChild size="lg" variant="outline"><Link to="/themes/preview">{lang === "ar" ? "شاهد التصاميم" : "Explore themes"}<ExternalLink className="ms-2" size={17} /></Link></Button></div><div className="mt-8 grid max-w-xl grid-cols-3 gap-4 text-sm"><div><strong className="block text-xl">5</strong><span className="text-muted-foreground">{lang === "ar" ? "تصاميم" : "Themes"}</span></div><div><strong className="block text-xl">RTL</strong><span className="text-muted-foreground">{lang === "ar" ? "من الأساس" : "Native"}</span></div><div><strong className="block text-xl">QR</strong><span className="text-muted-foreground">{lang === "ar" ? "ومشاركة" : "& sharing"}</span></div></div></div><div className="relative mx-auto w-full max-w-xl"><div className="rounded-[2rem] border bg-muted/40 p-3 shadow-2xl"><div className="overflow-hidden rounded-[1.5rem] border bg-background"><div className="aspect-[4/3] bg-gradient-to-br from-muted to-background p-5"><div className="flex items-center justify-between text-xs text-muted-foreground"><span>Menu V3</span><span>● Open</span></div><div className="mt-12 max-w-xs"><p className="text-xs uppercase tracking-[.22em] text-muted-foreground">Restaurant</p><h2 className="mt-2 text-4xl font-semibold">Taste your story.</h2><p className="mt-3 text-sm text-muted-foreground">{lang === "ar" ? "قائمة مختارة بعناية" : "A carefully curated menu"}</p></div><div className="mt-10 grid grid-cols-2 gap-3"><div className="aspect-square rounded-xl bg-muted" /><div className="aspect-square rounded-xl bg-muted" /></div></div></div></div></div></div></div></section>
      ...
      <section id="themes" className="px-4 py-20 sm:px-6"><div className="mx-auto max-w-7xl"><div className="flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><p className="text-sm font-medium">{lang === "ar" ? "هوية قبل القالب" : "Identity before template"}</p><h2 className="mt-3 text-3xl font-semibold sm:text-4xl">{lang === "ar" ? "اختر الأسلوب الذي يشبه مطعمك." : "Choose the style that fits your restaurant."}</h2></div><Button asChild variant="outline"><Link to="/themes/preview">{lang === "ar" ? "معاينة كاملة" : "Full preview"}</Link></Button></div><div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{themeCards.map((theme) => <article key={theme.key} className="group overflow-hidden rounded-2xl border bg-background"><div className="aspect-[4/3] overflow-hidden bg-muted"><img src={theme.preview.image} alt="" className="h-full w-full object-cover transition duration-500 group-hover:scale-105" loading="lazy" /></div><div className="p-5"><div className="flex items-center justify-between gap-3"><h3 className="text-xl font-semibold">{lang === "ar" ? theme.name.ar : theme.name.en}</h3><span className="text-xs text-muted-foreground">{theme.key}</span></div><p className="mt-2 text-sm leading-6 text-muted-foreground">{lang === "ar" ? theme.promise.ar : theme.promise.en}</p><Button className="mt-5 w-full" variant={selectedTheme === theme.key ? "default" : "outline"} onClick={() => choose("", theme.key)}>{selectedTheme === theme.key ? (lang === "ar" ? "التصميم مختار" : "Theme selected") : (lang === "ar" ? "اختر التصميم" : "Choose theme")}</Button></div></article>)}</div></div></section>
