      ...
      <section className="relative overflow-hidden px-4 pb-20 pt-16 sm:px-6 sm:pt-24">
        <div className="mx-auto grid max-w-7xl items-center gap-12 lg:grid-cols-[1.05fr_.95fr]">
          <div className="max-w-3xl">
            ...
            <h1 className="text-balance text-5xl font-semibold tracking-tight sm:text-6xl lg:text-7xl">{lang === "ar" ? "حوّل منيو مطعمك إلى تجربة تستحق الزيارة." : "Turn your restaurant menu into an experience worth visiting."}</h1>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <Button size="lg" onClick={() => choose()}>{lang === "ar" ? "ابدأ طلبك" : "Start your request"}<ArrowUpLeft className="ms-2" size={18} /></Button>
              <Button asChild size="lg" variant="outline"><Link to="/themes/preview">{lang === "ar" ? "شاهد التصاميم" : "Explore themes"}<ExternalLink className="ms-2" size={17} /></Link></Button>
            </div>
            <div className="mt-8 grid max-w-xl grid-cols-3 gap-4 text-sm">
              <div><strong className="block text-xl">5</strong><span className="text-muted-foreground">{lang === "ar" ? "تصاميم" : "Themes"}</span></div>
              <div><strong className="block text-xl">RTL</strong><span className="text-muted-foreground">{lang === "ar" ? "من الأساس" : "Native"}</span></div>
              <div><strong className="block text-xl">QR</strong><span className="text-muted-foreground">{lang === "ar" ? "ومشاركة" : "& sharing"}</span></div>
            </div>
          </div>
          <div className="relative mx-auto w-full max-w-xl">
            <div className="rounded-[2rem] border bg-muted/40 p-3 shadow-2xl"><div className="overflow-hidden rounded-[1.5rem] border bg-background">
              <div className="aspect-[4/3] bg-gradient-to-br from-muted to-background p-5">
                <div className="flex items-center justify-between text-xs text-muted-foreground"><span>Menu V3</span><span>● Open</span></div>
                <div className="mt-12 max-w-xs"><p className="text-xs uppercase tracking-[.22em] text-muted-foreground">Restaurant</p><h2 className="mt-2 text-4xl font-semibold">Taste your story.</h2><p className="mt-3 text-sm text-muted-foreground">{lang === "ar" ? "قائمة مختارة بعناية" : "A carefully curated menu"}</p></div>
                <div className="mt-10 grid grid-cols-2 gap-3"><div className="aspect-square rounded-xl bg-muted" /><div className="aspect-square rounded-xl bg-muted" /></div>
              </div>
            </div></div>
          </div>
        </div>
