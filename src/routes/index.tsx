...
import {
  ...
  Layers3, Menu, ScanLine, Sparkles, Smartphone, Store, TrendingUp, X, QrCode,
} from "lucide-react";
...
function Home() {
  ...
  return <main className="min-h-screen bg-background text-foreground">
    ...
    <section id="pricing" className="px-4 py-20 sm:px-6"><div className="mx-auto max-w-7xl"><div className="flex items-end justify-between gap-4"><div><p className="text-sm font-medium">{lang === "ar" ? "باقات واضحة" : "Simple plans"}</p><h2 className="mt-3 text-3xl font-semibold sm:text-4xl">{lang === "ar" ? "ابدأ بما يناسب حجم عملك." : "Start with the plan that fits your business."}</h2></div><div className="flex rounded-full border p-1 text-xs"><button className={`rounded-full px-3 py-1.5 ${billing === "monthly" ? "bg-foreground text-background" : ""}`} onClick={() => setBilling("monthly")}>{lang === "ar" ? "شهري" : "Monthly"}</button><button className={`rounded-full px-3 py-1.5 ${billing === "yearly" ? "bg-foreground text-background" : ""}`} onClick={() => setBilling("yearly")}>{lang === "ar" ? "سنوي" : "Yearly"}</button></div></div><div className="mt-10 grid gap-5 md:grid-cols-3">{COMMERCIAL_PLANS.map((plan, index) => <article key={plan.code} className={`rounded-2xl border p-6 ${index === 1 ? "ring-2 ring-foreground" : ""}`}><h3 className="text-xl font-semibold">{plan.name?.[lang] ?? plan.code}</h3><p className="mt-2 text-sm text-muted-foreground">{plan.description?.[lang] ?? ""}</p><div className="mt-6 text-4xl font-semibold">{billing === "yearly" ? (plan as any).yearlyPrice ?? (plan as any).price : (plan as any).monthlyPrice ?? (plan as any).price} <span className="text-sm font-normal text-muted-foreground">{lang === "ar" ? "ريال" : "SAR"}</span></div><ul className="mt-6 space-y-3 text-sm">{(plan.features ?? []).slice(0, 6).map((feature: any) => <li key={String(feature)} className="flex gap-2"><Check size={17}/><span>{typeof feature === "string" ? feature : feature?.[lang] ?? feature?.en ?? ""}</span></li>)}</ul><Button className="mt-7 w-full" onClick={() => choose(plan.code)}>{lang === "ar" ? "اختر الباقة" : "Choose plan"}</Button></article>)}</div></div></section>
...
function LeadForm({ lang, selectedPlan, selectedTheme }: { lang: "ar" | "en"; selectedPlan: string; selectedTheme: ThemeKey | "" }) {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await submitLead({ restaurantName: String(data.get("restaurantName") || ""), contactName: String(data.get("contactName") || ""), phone: String(data.get("phone") || ""), email: String(data.get("email") || ""), message: String(data.get("message") || ""), plan: selectedPlan, theme: selectedTheme });
      setStatus(lang === "ar" ? "تم إرسال الطلب بنجاح." : "Request submitted successfully.");
      event.currentTarget.reset();
    } catch {
      setStatus(lang === "ar" ? "تعذر إرسال الطلب. حاول مرة أخرى." : "Could not submit the request. Please try again.");
    }
  }
  return <form onSubmit={handleSubmit} className="rounded-2xl border bg-background p-6 shadow-sm"><div className="grid gap-5 sm:grid-cols-2"><Field label={lang === "ar" ? "اسم المطعم" : "Restaurant name"}><Input name="restaurantName" required /></Field><Field label={lang === "ar" ? "اسم التواصل" : "Contact name"}><Input name="contactName" required /></Field><Field label={lang === "ar" ? "رقم الجوال" : "Phone"}><Input name="phone" required /></Field><Field label={lang === "ar" ? "البريد الإلكتروني" : "Email"}><Input name="email" type="email" /></Field><div className="sm:col-span-2"><Field label={lang === "ar" ? "ملاحظات" : "Notes"}><Textarea name="message" rows={5} /></Field></div></div><Button type="submit" className="mt-6">{lang === "ar" ? "إرسال الطلب" : "Submit request"}</Button>{status && <p role="status" className="mt-3 text-sm text-muted-foreground">{status}</p>}</form>;
}
