...
import { approveLead, revokeLeadOnboarding } from "@/lib/menu/platform-onboarding";
...
function AdminOnboardingPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (!accounts.ok) { setError(accounts.error); setLoading(false); return; }
      if (!leads.ok) { setError(leads.error); setLoading(false); return; }
      const leadRows = leads.data.leads.map(mapLead);
      const accountRows = accounts.data.map((row) => ({ ...row, source: "account" as const }));
      const merged = [...accountRows, ...leadRows]
