...
function Onboarding() {
  ...
  const [selfServeEligible, setSelfServeEligible] = useState(false);
  ...
  const checkAccess = useCallback(async () => {
    ...
    try {
      ...
      const [access, activation, eligibility] = await Promise.all([getMyCustomerAccessStatus(), getMyActivationRequest(), getSelfServeWorkspaceEligibility()]);
      if (!access.ok) { setCheckError(access.error); return; }
      if (!activation.ok) { setCheckError(activation.error); return; }
      if (!eligibility.ok) { setCheckError(eligibility.error); return; }
      setStatus(access.data.status);
      setRequest(activation.data);
      setSelfServeEligible(eligibility.data.eligible);
    } catch (err) {
  ...
  if (status === "none" && selfServeEligible) {
...
function SelfServeWorkspaceSetup({ busy, setBusy, error, setError, onProvisioned }: { busy: boolean; setBusy: (value: boolean) => void; error: string; setError: (value: string) => void; onProvisioned: () => void }) {
  ...
  return <main dir={lang === "ar" ? "rtl" : "ltr"} className="grid min-h-dvh place-items-center bg-paper px-5 py-10 text-ink">
    <section className="w-full max-w-xl rounded-3xl border border-line bg-white p-6 shadow-sm md:p-8">
      ...
      <div className="mt-8 grid gap-2"><p className="text-sm font-medium text-accent">{lang === "ar" ? "الخطوة الثانية" : "Step 2"}</p><h1 className="font-display text-2xl font-semibold">{lang === "ar" ? "جهّز مساحة عملك" : "Set up your workspace"}</h1><p className="text-sm leading-6 text-muted">{lang === "ar" ? "نحتاج المعلومات الأساسية فقط للبدء. يمكنك إكمال الشعار وباقي التفاصيل لاحقًا من الاستوديو." : "We only need the essentials to get started. You can complete the logo and other details later in Studio."}</p></div>
...
function CustomerLifecycleView({ lang, status, request, busy, setBusy, error, setError, refresh }: { lang: "ar" | "en"; status: AccessStatus; request: CustomerActivationRequest | null; busy: boolean; setBusy: (value: boolean) => void; error: string; setError: (value: string) => void; refresh: () => Promise<void> }) {
  ...
  const types: Array<[BusinessType, string, string]> = [["restaurant", "مطعم", "Restaurant"], ["cafe", "كافيه", "Café"], ["bakery", "مخبز", "Bakery"], ["dessert", "حلويات", "Dessert"], ["food_truck", "عربة طعام", "Food truck"], ["other", "نشاط آخر", "Other"]];

  useEffect(() => { if (!request) return; setBusinessType(request.businessType); setBrandNameAr(request.brandNameAr); setBrandNameEn(request.brandNameEn); setDescriptionAr(request.descriptionAr); }, [request?.id]);

  async function submit(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault(); if (busy) return; setBusy(true); setError("");
    try { const result = await submitActivationRequest({ data: { nameAr: brandNameAr, nameEn: brandNameEn || undefined, businessType, descriptionAr: descriptionAr || undefined } }); if (!result.ok) throw new Error(result.error); await refresh(); }
    catch (err) { setError(err instanceof Error ? err.message : (lang === "ar" ? "تعذر إرسال الطلب" : "Could not submit the request")); }
    finally { setBusy(false); }
  }

  async function activate() {
    if (busy) return; setBusy(true); setError("");
    try { const result = await activateApprovedWorkspace(); if (!result.ok) throw new Error(result.error); window.location.href = "/studio"; }
    catch (err) { setError(err instanceof Error ? err.message : (lang === "ar" ? "تعذر تفعيل مساحة العمل" : "Could not activate the workspace")); }
    finally { setBusy(false); }
  }

  const copy = lang === "ar" ? {
    title: "تفعيل مساحة عملك", noneTitle: "أرسل طلب التفعيل", noneBody: "أنشئ حسابك أولًا. بعد إرسال بيانات البراند سيظهر الطلب مباشرة في مركز اعتماد المنصة. لن يتم إنشاء مساحة مطعم قبل موافقة الإدارة.", pendingTitle: "طلبك قيد المراجعة", pendingBody: "تم استلام طلب التفعيل. يمكنك تسجيل الدخول لاحقًا لمتابعة الحالة؛ لا يمكن دخول الاستوديو قبل الاعتماد.", actionTitle: "نحتاج معلومات إضافية", actionBody: "راجعت الإدارة طلبك وتحتاج إلى تعديل البيانات قبل إعادة إرساله.", approvedTitle: "تم اعتماد طلبك", approvedBody: "تم اعتماد الطلب. فعّل مساحة العمل الآن، وبعدها يمكنك الدخول إلى الاستوديو وإكمال إعداد البراند والمنيو.", rejectedTitle: "لم تتم الموافقة على الطلب", rejectedBody: "يمكنك إرسال الطلب مرة أخرى بعد مراجعة ملاحظات الإدارة.", convertedTitle: "مساحتك جاهزة", convertedBody: "تم تفعيل مساحة العمل لهذا الحساب.", brand: "Menu V3", brandName: "اسم البراند", english: "الاسم بالإنجليزية (اختياري)", type: "نوع النشاط", description: "وصف مختصر (اختياري)", submit: request ? "إعادة إرسال طلب التفعيل" : "إرسال طلب التفعيل", activate: "تفعيل مساحة العمل", studio: "فتح الاستوديو", refresh: "تحديث الحالة", reason: request?.decisionReason ? `ملاحظة الإدارة: ${request.decisionReason}` : ""
  } : {
    title: "Activate your workspace", noneTitle: "Submit activation request", noneBody: "Create your account first. After you submit your brand details, the request appears in Platform Admin. No restaurant workspace is created before approval.", pendingTitle: "Your request is under review", pendingBody: "Your activation request was received. You can sign in again later to check the status; Studio remains unavailable until approval.", actionTitle: "Action required", actionBody: "The platform team reviewed your request and needs updated information before it can be resubmitted.", approvedTitle: "Your request is approved", approvedBody: "Your request is approved. Activate the workspace now, then enter Studio to finish your brand and menu setup.", rejectedTitle: "The request was not approved", rejectedBody: "You can submit the request again after reviewing the platform team's notes.", convertedTitle: "Your workspace is ready", convertedBody: "This account already has an activated workspace.", brand: "Menu V3", brandName: "Brand name", english: "English name (optional)", type: "Business type", description: "Short description (optional)", submit: request ? "Resubmit activation request" : "Submit activation request", activate: "Activate workspace", studio: "Open Studio", refresh: "Refresh status", reason: request?.decisionReason ? `Admin note: ${request.decisionReason}` : ""
  };

  const showForm = status === "none" || status === "action_required" || status === "rejected";
  const title = status === "none" ? copy.noneTitle : status === "pending" ? copy.pendingTitle : status === "action_required" ? copy.actionTitle : status === "approved" ? copy.approvedTitle : status === "rejected" ? copy.rejectedTitle : copy.convertedTitle;
  const body = status === "none" ? copy.noneBody : status === "pending" ? copy.pendingBody : status === "action_required" ? copy.actionBody : status === "approved" ? copy.approvedBody : status === "rejected" ? copy.rejectedBody : copy.convertedBody;

  return <main dir={lang === "ar" ? "rtl" : "ltr"} className="grid min-h-dvh place-items-center bg-paper px-5 py-10 text-ink"><section className="w-full max-w-xl rounded-3xl border border-line bg-white p-6 shadow-sm md:p-8">
    ...
    <div className="mt-8 grid gap-2"><p className="text-sm font-medium text-accent">{copy.title}</p><h1 className="font-display text-2xl font-semibold">{title}</h1><p className="text-sm leading-6 text-muted">{body}</p>{copy.reason ? <p className="rounded-xl border border-line bg-sand/30 p-3 text-sm leading-6">{copy.reason}</p> : null}</div>
    ...
    <p className="mt-5 text-xs leading-5 text-muted">{lang === "ar" ? "مسار اعتماد العملاء الحالي محفوظ للحسابات المرتبطة بطلبات سابقة. الحسابات الجديدة التي أكملت التسجيل تستخدم الإعداد الذاتي الآمن." : "The existing approval path remains protected for accounts linked to prior requests. Newly registered accounts use secure self-serve setup."}</p>
  </section></main>;
}
