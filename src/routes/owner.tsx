...
const LEAD_LABELS: Record<LeadStatus, string> = { new: "جديد", contacted: "تم التواصل", qualified: "مؤهل", converted: "تم التحويل", lost: "مغلق" };
...
function selectedOptionText(value: unknown) {
  if (!Array.isArray(value)) return "";
  return value.map((item) => {
    if (!item || typeof item !== "object") return "";
    const option = item as { nameAr?: string; nameEn?: string; type?: string };
    return option.nameAr || option.nameEn || "";
  }).filter(Boolean).join(" · ");
}
...
function OwnerPage() {
  ...
  async function load() {
    ...
    try {
      ...
      if (!orderResult.ok) setError(orderResult.error); else { setOrders(orderResult.data); setSelectedOrder((s) => s && orderResult.data.orders.some((o) => o.id === s.id) ? orderResult.data.orders.find((o) => o.id === s.id) ?? s : orderResult.data.orders[0] ?? null); }
      if (!leadResult.ok) setError((current) => current || leadResult.error); else { setLeads(leadResult.data); setSelectedLead((s) => s && leadResult.data.leads.some((l) => l.id === s.id) ? leadResult.data.leads.find((l) => l.id === s.id) ?? s : leadResult.data.leads[0] ?? null); }
  ...
  }, [isPending, user, orderStatus, leadStatus, query]);

  async function changeOrder(id: string, status: OrderStatus) {
    const result = await updateOrderStatus({ data: { id, status } });
    if (!result.ok) { setError(result.error); return; }
    setOrders((d) => ({ ...d, orders: d.orders.map((o) => o.id === id ? result.data : o) })); setSelectedOrder(result.data);
  }
  async function changeLead(lead: AdminLead, status: LeadStatus) {
    ...
    if (!result.ok) { setError(result.error); return; }
    setLeads((d) => ({ ...d, leads: d.leads.map((l) => l.id === lead.id ? result.data : l) })); setSelectedLead(result.data);
  }
  ...
  return <main className="mx-auto grid max-w-7xl gap-6 py-4 lg:py-8">
    <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between"><div><span className="inline-flex items-center gap-2 rounded-full border border-line bg-sand/50 px-3 py-1 text-xs text-muted"><ShoppingBag className="size-3.5" /> Owner Workspace</span><h1 className="mt-2 font-display text-3xl font-semibold tracking-tight sm:text-4xl">مركز الطلبات والعملاء المحتملين</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-ink-soft">مساحة تشغيل المالك: تابع الطلبات الفعلية، حدّث حالتها، وأدر العملاء المحتملين من مكان واحد. تحرير المنيو والمنتجات يبقى داخل Studio.</p></div><Button variant="outline" onClick={() => void load()} disabled={loading}><RefreshCw className={cn("size-4", loading && "animate-spin")} /> تحديث</Button></header>
    <section className="grid grid-cols-2 gap-3 md:grid-cols-5"><Metric label="الطلبات" value={orders.total} active={tab === "orders"} onClick={() => setTab("orders")} icon={<ShoppingBag className="size-4" />} /><Metric label="جديدة" value={orders.newCount} active={tab === "orders" && orderStatus === "new"} onClick={() => { setTab("orders"); setOrderStatus(orderStatus === "new" ? "all" : "new"); }} /><Metric label="نشطة" value={orders.activeCount} active={tab === "orders"} icon={<ShoppingBag className="size-4" />} /><Metric label="العملاء المحتملون" value={leads.total} active={tab === "leads"} onClick={() => setTab("leads")} icon={<Users className="size-4" />} /><Metric label="عملاء مؤهلون" value={leads.qualifiedCount} active={tab === "leads" && leadStatus === "qualified"} onClick={() => { setTab("leads"); setLeadStatus(leadStatus === "qualified" ? "all" : "qualified"); }} /></section>
