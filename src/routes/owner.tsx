...
function OwnerPage() {
  ...
  return <main className="mx-auto grid max-w-7xl gap-6 py-4 lg:py-8">
    ...
    <section className="grid grid-cols-2 gap-3 md:grid-cols-5"><Metric label="الطلبات" value={orders.total} active={tab === "orders"} onClick={() => setTab("orders")} icon={<ShoppingBag className="size-4" />} /><Metric label="جديدة" value={orders.newCount} active={tab === "orders" && orderStatus === "new"} onClick={() => { setTab("orders"); setOrderStatus(orderStatus === "new" ? "all" : "new"); }} /><Metric label="نشطة" value={orders.activeCount} active={tab === "orders"} onClick={() => setTab("orders")} /><Metric label="العملاء المحتملون" value={leads.total} active={tab === "leads"} onClick={() => setTab("leads")} icon={<Users className="size-4" />} /><Metric label="عملاء مؤهلون" value={leads.qualifiedCount} active={tab === "leads" && leadStatus === "qualified"} onClick={() => { setTab("leads"); setLeadStatus(leadStatus === "qualified" ? "all" : "qualified"); }} /></section>
