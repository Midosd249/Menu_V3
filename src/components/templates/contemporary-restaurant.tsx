...
function ProductDialog({ lang, product, options, close, add, submitting }: { lang: Lang; product: Product; options?: ProductOptions; close: () => void; add: (item: CartItem) => void; submitting: boolean }) {
  ...
  return <div className="fixed inset-0 z-[60] grid items-end bg-black/55 p-0 sm:items-center sm:p-5" onMouseDown={(e) => e.target === e.currentTarget && close()} role="presentation"><section role="dialog" aria-modal="true" aria-labelledby="editorial-product-title" className="editorial-dialog max-h-[94dvh] w-full overflow-y-auto bg-paper shadow-2xl sm:mx-auto sm:max-w-xl"><div className="sticky top-0 z-10 flex items-center justify-between border-b border-line bg-paper/95 px-5 py-4 backdrop-blur"><span className="text-xs font-semibold uppercase tracking-[0.16em]">{text(lang, "تفاصيل الطبق", "Dish details")}</span><button type="button" onClick={close} aria-label={text(lang, "إغلاق", "Close")} className="grid size-11 place-items-center rounded-full border border-line hover:bg-sand focus-visible:outline-2"><X className="size-5" /></button></div><div className="grid gap-5 p-5"><MenuMedia
          src={product.imageUrl}
          alt={text(lang, product.nameAr, product.nameEn)}
          imageWidth={960}
...
export function ContemporaryRestaurantTemplate({ menu, preview = false }: Props) {
  ...
  return <div className="menu-public-shell min-h-dvh bg-paper text-ink" data-editorial-root="true" style={{ "--menu-accent": tenant.accentColor, "--menu-ink": tenant.primaryColor } as React.CSSProperties}>
    ...
    <header className="editorial-hero" data-editorial-layer="hero"><div className="editorial-hero-media"><MenuMedia
      src={tenant.coverUrl}
      alt=""
      imageWidth={1600}
      ...
      eager
      fallback={<span aria-hidden>{tenant.nameAr.slice(0, 1)}</span>}
    /></div><div className="editorial-hero-shade" aria-hidden /><div className="editorial-hero-inner"><div className="editorial-kicker"><span>{text(lang, "المنيو", "MENU")}</span><span>VOL. 03</span><span>{tenant.city}</span></div><div className="editorial-brand-row"><MenuMedia
        src={tenant.logoUrl}
        alt=""
        imageWidth={240}
        imageWidths={[96, 160, 240]}
        sizes="(min-width: 701px) 160px, 96px"
        eager
        fallback={tenant.nameAr.slice(0, 1)}
        className="editorial-brand-logo"
      /><div className="min-w-0"><p className="editorial-branch">{text(lang, branch.nameAr, branch.nameEn)}</p><h1>{text(lang, tenant.nameAr, tenant.nameEn)}</h1>{tenant.taglineAr || tenant.taglineEn ? <p className="editorial-tagline">{text(lang, tenant.taglineAr, tenant.taglineEn)}</p> : null}</div></div><div className="editorial-hero-meta">{status !== null ? <><span>{status ? text(lang, "مفتوح الآن", "Open now") : text(lang, "مغلق", "Closed")}</span><span aria-hidden>•</span></> : null}<span>{tenant.city || text(lang, "السعودية", "Saudi Arabia")}</span></div></div></header>
    ...
    <main className="editorial-main">
      {featured.length && categoryId === "all" && !query ? <section className="editorial-selection" aria-labelledby="editorial-selection-title"><div className="editorial-section-heading"><div><span>01</span><p>{text(lang, "مختارات المحرر", "Editor's selection")}</p><h2 id="editorial-selection-title">{text(lang, "ابدأ بهذه الأطباق", "Start with these")}</h2></div><span className="editorial-heading-rule" /></div><div className="editorial-featured-grid">{featured.slice(0, 6).map((p, index) => { const decision = getQuickAddDecision(p, menu.productOptions?.[p.id]); return <article key={p.id} className={cn("editorial-featured-card-wrap", index === 0 && "is-lead")}><button type="button" onClick={() => selectProduct(p)} className={cn("editorial-featured-card", index === 0 && "is-lead")}><MenuMedia
                ...
                imageQuality={76}
                imageFit="crop"
                sizes="(min-width: 1100px) 42vw, (min-width: 700px) 50vw, 100vw"
                eager={index < 2}
      ...
      {filtered.length === 0 ? <section className="editorial-empty"><EmptyState title={text(lang, "لا توجد أصناف مطابقة", "No matching items")} /></section> : (categoryId === "all" ? categories : categories.filter((c) => c.id === categoryId)).map((category, categoryIndex) => { const items = filtered.filter((p) => p.categoryId === category.id); if (!items.length) return null; return <section key={category.id} id={`category-${category.id}`} className="editorial-category" aria-labelledby={`category-${category.id}-title`}><div className="editorial-section-heading"><div><span>{String(categoryIndex + 2).padStart(2, "0")}</span><p>{text(lang, "قسم", "Section")}</p><h2 id={`category-${category.id}-title`}>{text(lang, category.nameAr, category.nameEn)}</h2></div><span className="editorial-heading-rule" /></div><ul>{items.map((p, index) => { const decision = getQuickAddDecision(p, menu.productOptions?.[p.id]); return <li key={p.id} className="editorial-product-row"><div className="editorial-product-card-wrap"><button type="button" disabled={!p.isAvailable && !preview} onClick={() => selectProduct(p)} className="editorial-product-card"><MenuMedia
                  ...
                  imageQuality={76}
                  imageFit="crop"
                  sizes="(min-width: 1100px) 448px, (min-width: 701px) 320px, 192px"
