    ...
    <div className="taste-dialog-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && close()}>
      <section role="dialog" aria-modal="true" aria-labelledby="taste-product-title" className="taste-dialog">
        ...
        <div className="taste-dialog-body">
          <MenuMedia src={product.imageUrl} alt={text(lang, product.nameAr, product.nameEn)} imageWidth={960} imageWidths={[480, 640, 960]} sizes="(min-width: 640px) 32rem, calc(100vw - 2rem)" className="taste-dialog-image" eager fallback={text(lang, "صورة الصنف", "Dish image")} />
  ...
  return (
    <div className="taste-page" dir={lang === "ar" ? "rtl" : "ltr"}>
      ...
      <header className="taste-hero">
        <MenuMedia src={tenant.coverUrl} alt={text(lang, tenant.nameAr, tenant.nameEn)} imageWidth={1600} imageWidths={[640, 960, 1280, 1600]} sizes="100vw" className="taste-hero-image" eager fallback={text(lang, "صورة المطعم", "Restaurant cover")} />
        <div className="taste-hero-overlay" />
        <div className="taste-hero-content">
          <div className="taste-hero-copy"><div className="taste-hero-logo-wrap">{tenant.logoUrl ? <MenuMedia src={tenant.logoUrl} alt={text(lang, tenant.nameAr, tenant.nameEn)} imageWidth={240} imageWidths={[96, 160, 240]} sizes="96px" className="taste-hero-logo" eager fallback={tenant.nameAr.slice(0, 1)} /> : null}</div><span className="taste-eyebrow">{text(lang, "تجربة طعام عربية معاصرة", "Contemporary Arabic dining")}</span><h1>{text(lang, tenant.taglineAr || tenant.nameAr, tenant.taglineEn || tenant.nameEn)}</h1><p>{text(lang, tenant.taglineAr, tenant.taglineEn)}</p><button type="button" onClick={() => document.getElementById("taste-menu")?.scrollIntoView({ behavior: "smooth", block: "start" })} className="taste-hero-button">{text(lang, "اطلب الآن", "Order now")}</button></div>
