...
export function SignalTableTemplate({ menu, preview = false }: Props) {
  ...
  const [query, setQuery] = useState(""); const [categoryId, setCategoryId] = useState("all"); const [selectedId, setSelectedId] = useState<string | null>(null); const [cart, setCart] = useState<CartItem[]>([]); const [cartOpen, setCartOpen] = useState(false); const [submitting, setSubmitting] = useState(false); const [error, setError] = useState(""); const [success, setSuccess] = useState<{ id: string; number: number; total: number } | null>(null); const searchTrackedRef = useRef(false);
  ...
  return <div className="menu-public-shell min-h-dvh bg-paper text-ink" data-signal-root="true" style={{ "--menu-accent": tenant.accentColor, "--menu-ink": tenant.primaryColor } as React.CSSProperties}>
    ...
    <main className="signal-main">
      {categoryId === "all" && !query && featured.length ? <section className="signal-featured" aria-labelledby="signal-featured-title"><div className="signal-section-heading"><div><p>{text(lang, "اختيار المطعم", "Restaurant picks")}</p><h2 id="signal-featured-title">{text(lang, "الأكثر تميزاً", "Featured picks")}</h2></div><span className="signal-heading-rule" /></div><div className="signal-featured-grid">{featured.slice(0, 4).map((p) => <article key={p.id} className="signal-featured-card-wrap"><button type="button" disabled={!p.isAvailable && !preview} onClick={() => selectProduct(p)} className="signal-featured-card"><MenuMedia src={p.imageUrl} alt="" className="signal-featured-image" fallback={text(lang, "صورة الطبق", "Dish image")} /><span className="signal-card-copy"><span className="signal-card-title">{text(lang, p.nameAr, p.nameEn)}</span>{p.descriptionAr || p.descriptionEn ? <span className="signal-card-description">{text(lang, p.descriptionAr, p.descriptionEn)}</span> : null}<span className="signal-card-price-row"><MenuPrice price={p.price} currency={p.currency} lang={lang} className="signal-card-price" /></span></span></button></article>)}</div></section> : null}
    ...
    {success ? <div className="fixed inset-0 z-[70] grid place-items-center bg-black/55 p-4" role="presentation"><section role="dialog" aria-modal="true" aria-labelledby="signal-success-title" className="signal-success w-full max-w-sm bg-paper p-6 text-center shadow-2xl"><h2 id="signal-success-title" className="font-display text-xl font-semibold">{text(lang, "تم استلام طلبك", "Order received")}</h2><p className="mt-2 text-sm text-muted">{text(lang, "رقم الطلب", "Order number")} <strong className="text-ink">#{success.number}</strong></p><p className="mt-1 text-sm text-muted">{formatSar(success.total, lang)}</p><div className="mt-5 grid gap-2"><OrderReceiptButton orderId={success.id} className="w-full" /><button type="button" onClick={() => setSuccess(null)} className="mt-5 h-11 w-full rounded-full bg-ink text-paper">{text(lang, "العودة للمنيو", "Back to menu")}</button></div></section></div> : null}
  </div>;
}
