...
function DishMedia({
  ...
  const { src, srcSet } = getResponsiveImageSources(product.imageUrl, {
    width: imageWidth,
    widths: imageWidths,
    quality: 76,
    fit: "crop",
  ...
  return <img src={src} srcSet={srcSet} alt="" loading={eager ? "eager" : "lazy"} decoding="async" fetchPriority={fetchPriority} sizes={srcSet ? sizes : undefined} className={cn("object-cover", className)} onError={() => setFailed(true)} />;
}
...
function CartDrawer({ tenant, branch, lang, items, setItems, close, submit, submitting, error }: { tenant: PublicMenu["tenant"]; branch: PublicMenu["branch"]; lang: Lang; items: CartItem[]; setItems: (items: CartItem[]) => void; close: () => void; submit: (customer: { name: string; phone: string; email: string; notes: string }) => void; submitting: boolean; error: string }) {
  ...
  return <div className="fixed inset-0 z-[60] bg-black/45" role="presentation" onMouseDown={(e) => { if (e.target === e.currentTarget) close(); }}>
    <section ref={dialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="cart-title" className="ms-auto flex h-full w-full max-w-lg flex-col bg-paper shadow-2xl">
      ...
      {items.length ? <div className="border-t border-line bg-paper p-4"><div className="mb-3 flex items-center justify-between"><span className="text-sm text-muted">{label(lang, "الإجمالي", "Total")}</span><strong className="text-lg text-accent"><bdi dir="ltr" className="tabular bidi-isolate">{formatSar(total, lang)}</bdi></strong></div><div className="grid gap-2">{whatsappOrder ? <a href={whatsappOrder} target="_blank" rel="noreferrer" onClick={trackWhatsApp} className="inline-flex h-12 w-full items-center justify-center gap-2 rounded-xl border border-success/30 bg-success/10 font-medium text-ink"><WhatsAppIcon />{label(lang, "إرسال الطلب عبر واتساب", "Send order via WhatsApp")}</a> : null}<button type="button" disabled={submitting || name.trim().length < 2 || phone.trim().length < 8} onClick={() => submit({ name, phone, email, notes })} className="h-12 w-full rounded-xl bg-ink font-medium text-paper disabled:opacity-50">{submitting ? label(lang, "جاري إرسال الطلب…", "Submitting…") : label(lang, "تأكيد إرسال الطلب", "Submit order")}</button></div></div> : null}
    </section>
  </div>;
}
...
export function PublicMenuView({ menu, preview = false }: { menu: PublicMenu; preview?: boolean }) {
  ...
  return <div className="menu-public-shell min-h-dvh bg-paper text-ink" style={{ "--menu-accent": tenant.accentColor, "--menu-ink": tenant.primaryColor } as CSSProperties}>
    ...
    <header className="relative overflow-hidden bg-ink text-paper"><div className="absolute inset-0 opacity-45" style={{ background: tenant.coverUrl ? `center/cover url(${tenant.coverUrl})` : `radial-gradient(100% 80% at 85% 0%, ${tenant.accentColor} 0%, transparent 55%)` }} aria-hidden="true" /><div className="relative mx-auto grid max-w-2xl gap-5 px-5 pb-9 pt-6">
      ...
      {tenant.taglineAr || tenant.taglineEn ? <p className="text-sm text-paper/80" dir="auto">{value(lang, tenant.taglineAr, tenant.taglineEn)}</p> : null}<div className="flex flex-wrap gap-2 text-xs">{hours.length ? <span className="rounded-full bg-paper/10 px-3 py-1">{status ? label(lang, "مفتوح الآن", "Open now") : label(lang, "مغلق", "Closed")}</span> : null}<span className="rounded-full bg-paper/10 px-3 py-1" dir="auto">{value(lang, branch.nameAr, branch.nameEn)}</span></div>
      <div className="flex flex-wrap gap-2">{wa ? <a href={wa} onClick={trackWhatsApp} className="inline-flex min-h-10 items-center gap-2 rounded-xl bg-[var(--menu-accent)] px-3 text-sm font-medium"><WhatsAppIcon />{label(lang, "واتساب", "WhatsApp")}</a> : null}{branch.mapsUrl ? <a href={branch.mapsUrl} className="inline-flex min-h-10 items-center gap-2 rounded-xl bg-paper/10 px-3 text-sm"><MapPin className="size-4" aria-hidden="true" />{label(lang, "الموقع", "Location")}</a> : null}{branch.phone ? <a href={`tel:${branch.phone}`} dir="ltr" className="inline-flex min-h-10 items-center gap-2 rounded-xl bg-paper/10 px-3 text-sm"><Phone className="size-4" aria-hidden="true" />{label(lang, "اتصال", "Call")}</a> : null}{tenant.instagramUrl ? <a href={tenant.instagramUrl} className="inline-flex min-h-10 items-center gap-2 rounded-xl bg-paper/10 px-3 text-sm"><Instagram className="size-4" aria-hidden="true" />Instagram</a> : null}</div>
    </div></header>
    ...
    {cartOpen ? <CartDrawer tenant={tenant} branch={branch} lang={lang} items={cart} setItems={setCart} close={() => setCartOpen(false)} submit={submit} submitting={submitting} error={orderError} /> : null}
    {successOrder ? <div className="fixed inset-0 z-[70] grid place-items-center bg-black/45 p-4" role="presentation"><section role="alertdialog" aria-modal="true" aria-labelledby="order-success-title" className="w-full max-w-sm rounded-2xl bg-paper p-6 text-center shadow-2xl"><div className="mx-auto mb-4 grid size-14 place-items-center rounded-full bg-sand text-xl" aria-hidden="true">✓</div><h2 id="order-success-title" className="text-xl font-semibold">{label(lang, "تم استلام طلبك", "Order received")}</h2><p className="mt-2 text-sm text-muted">{label(lang, "رقم الطلب", "Order number")} <strong className="text-ink"><bdi dir="ltr" className="tabular bidi-isolate">#{successOrder.number}</bdi></strong></p><p className="mt-1 text-sm text-muted"><bdi dir="ltr" className="tabular bidi-isolate">{formatSar(successOrder.total, lang)}</bdi></p><div className="mt-5 grid gap-2"><OrderReceiptButton orderId={successOrder.id} className="w-full" /><button type="button" autoFocus onClick={() => setSuccessOrder(null)} className="h-11 w-full rounded-xl bg-ink text-sm font-medium text-paper">{label(lang, "العودة للمنيو", "Back to menu")}</button></div></section></div> : null}
  </div>;
}
