    ...
    <div className="min-h-dvh bg-paper lg:grid lg:grid-cols-[240px_1fr]">
      <aside className="hidden border-e border-line lg:flex lg:flex-col">
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  active ? "bg-ink text-paper" : "text-ink-soft hover:bg-sand",
                )}
              >
        ...
        <div className="mt-auto grid gap-3 border-t border-line p-4">
          <Link to="/studio/preview" className="inline-flex items-center gap-2 text-sm text-ink-soft">
            <ExternalLink className="size-4" />
            {t(copy.nav.preview, lang)}
          </Link>
          {tenant.isPublished ? (
            <a href={publicHref} className="inline-flex items-center gap-2 text-sm text-ink-soft">
              <ExternalLink className="size-4" />
              {t(copy.studio.openMenu, lang)}
            </a>
          ) : null}
          <UserButton />
        </div>
      ...
      <div className="flex min-w-0 flex-col">
        <header className="flex items-center justify-between gap-3 border-b border-line px-4 py-3 lg:px-8">
          <div className="min-w-0">
            <p className="truncate text-sm font-medium">{lang === "ar" ? tenant.nameAr : tenant.nameEn || tenant.nameAr}</p>
        ...
        <div className="flex-1 px-4 py-6 pb-28 lg:px-8">
          <Outlet />
        </div>
          ...
          {NAV.filter((item) => (MOBILE_PRIMARY as readonly string[]).includes(item.to)).map((item) => {
            ...
            return (
              <Link
                ...
                to={item.to}
                className={cn(
                  "grid h-12 place-items-center rounded-md text-xs",
              ...
              {NAV.filter((item) => !(MOBILE_PRIMARY as readonly string[]).includes(item.to)).map((item) => {
                ...
                return (
                  <Link
                    ...
                    className={cn(
                      "grid h-20 place-items-center gap-1 rounded-lg border border-line text-xs",
