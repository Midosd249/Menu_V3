...
import { isThemeKey } from "./registry";
...
export const saveTenantTheme = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }): Promise<FnResult<{ themeKey: ThemeKey }>> => {
    if (!isThemeKey(data.themeKey)) return { ok: false, code: "invalid", error: "القالب غير صالح" };
    try {
      ...
      const rows = await sql<{ theme_key: string; slug: string }>`
        update tenants set theme_key = ${data.themeKey}, updated_at = now()
        where id = ${member.tenant_id} returning theme_key, slug
      `;
      const saved = rows[0]?.theme_key;
      const slug = rows[0]?.slug;
      if (!isThemeKey(saved) || !slug) return { ok: false, code: "unavailable", error: "تعذر تأكيد حفظ القالب" };
