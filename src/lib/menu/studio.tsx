...
type StudioCtx = { snapshot: StudioSnapshot; reload: () => Promise<void>; setSnapshot: (next: StudioSnapshot) => void };
...
function isStudioSnapshot(value: unknown): value is StudioSnapshot {
  return Boolean(value && typeof value === "object" && "tenant" in value && "products" in value && "branches" in value);
}
  ...
  const run = useCallback(
    async <T,>(fn: () => Promise<FnResult<T>>): Promise<boolean> => {
      ...
      try {
        ...
        if (isStudioSnapshot(result.data)) setSnapshot(result.data);
        setOk(true);
        return true;
      } catch (err) {
...
export function StudioGate({ children }: { children: ReactNode }) {
  ...
  if (sessionError && !user) return <ErrorState message={sessionError} onRetry={refreshSession} />;
