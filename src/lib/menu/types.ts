...
export type FnErr = {
  ok: false;
  error: string;
  code: "not_found" | "unauthorized" | "forbidden" | "unavailable" | "invalid";
};
...
export type MenuHealth = {
  score: number;
  checks: Array<{ key: string; ok: boolean; labelAr: string; labelEn: string }>;
...
export type OwnerAnalytics = {
  ...
  topProducts: AnalyticsTop[];
  byCategory: AnalyticsTop[];
  byBranch: AnalyticsTop[];
};
