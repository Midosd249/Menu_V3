...
export type Tenant = { id: string; ownerUserId: string; slug: string; nameAr: string; nameEn: string; taglineAr: string; taglineEn: string; logoUrl: string; coverUrl: string; instagramUrl: string; websiteUrl?: string; snapchatUrl?: string; facebookUrl?: string; tiktokUrl?: string; vatRegistrationNumber?: string; whatsapp: string; whatsappTemplate: string; primaryColor: string; accentColor: string; themeKey: ThemeKey; currency: string; city: string; country: string; isPublished: boolean; isActive: boolean; createdAt: string; updatedAt: string };
export type PublicTenant = Omit<Tenant, "ownerUserId" | "isPublished" | "isActive" | "createdAt" | "updatedAt" | "vatRegistrationNumber">;
...
export type CaffeineBasis = "per_100ml" | "per_cup";
export type Product = { id: string; tenantId: string; categoryId: string | null; sortOrder: number; nameAr: string; nameEn: string; descriptionAr: string; descriptionEn: string; price: number; currency: string; imageUrl: string; calories: number | null; sodiumMg?: number | null; caffeineMg?: number | null; caffeineBasis?: CaffeineBasis | null; isAvailable: boolean; isFeatured: boolean; allergens: string; tags: string[]; dietaryLabels: string[] };
...
export function hasHighSalt(product: Pick<Product, "sodiumMg">): boolean { return product.sodiumMg != null && product.sodiumMg >= 2000; }
export type ImportRow = { nameAr: string; nameEn: string; categoryAr: string; categoryEn: string; descriptionAr: string; descriptionEn: string; price: number; imageUrl: string; calories: number | null; sodiumMg?: number | null; caffeineMg?: number | null; caffeineBasis?: CaffeineBasis | null; isFeatured: boolean; isAvailable: boolean; tags?: string[]; dietaryLabels?: string[]; issues: string[] };
