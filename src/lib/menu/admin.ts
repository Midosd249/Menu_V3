...
function mapActivation(row: Record<string, unknown>): AdminActivationRequest {
  return {
    id: String(row.id),
    brandNameAr: String(row.business_name ?? ""),
    brandNameEn: String(row.brand_name_en ?? ""),
    businessType: String(row.business_type ?? "restaurant"),
...
export const getAdminDashboard = createServerFn({ method: "GET" })
  ...
  .validator(z.object({ status: z.enum(LEAD_STATUSES).optional(), q: searchSchema }))
...
export const getAdminActivationRequests = createServerFn({ method: "GET" })
  .middleware([authMiddleware])
  .validator(z.object({ status: z.enum(ACTIVATION_STATUSES).optional(), q: searchSchema }))
  .handler(async ({ context, data }): Promise<FnResult<AdminActivationRequest[]>> => {
    const permission = await assertAdmin(context.userId);
    if (!permission.ok) return permission;
    try {
      const sql = await getSql();
      ...
      const rows = await sql<Record<string, unknown>>`
        ...
        where activation_status <> 'legacy'
          and (${data.status ?? null}::text is null or activation_status = ${data.status ?? null})
          and (${q}::text is null or lower(business_name) like ${q} or lower(contact_name) like ${q} or lower(coalesce(contact_email, '')) like ${q} or contact_phone like ${q})
        order by activation_requested_at desc nulls last, updated_at desc
        limit 100
      `;
      return { ok: true, data: rows.map(mapActivation) };
    } catch (err) {
      console.error("getAdminActivationRequests failed", err);
      return { ok: false, code: "unavailable", error: "تعذر تحميل طلبات التفعيل" };
    }
  });

export const reviewActivationRequest = createServerFn({ method: "POST" })
  .middleware([authMiddleware])
  .validator(z.object({
    id: z.string().min(1).max(100),
    decision: z.enum(["approve", "reject", "action_required"]),
  ...
  .handler(async ({ context, data }): Promise<FnResult<AdminActivationRequest>> => {
    ...
    try {
      const sql = await getSql();
      const nextStatus = data.decision === "approve" ? "approved" : data.decision === "reject" ? "rejected" : "action_required";
      const rows = await sql<Record<string, unknown>>`
        update leads
        set activation_status = ${nextStatus},
            status = ${data.decision === "approve" ? "qualified" : data.decision === "reject" ? "lost" : "contacted"},
            decision_at = now(),
            decision_by = ${context.userId},
            decision_reason = ${data.reason ?? ""},
            updated_at = now()
        where id = ${data.id}
          and activation_status in ('pending', 'action_required', 'rejected')
        returning *
      `;
      if (!rows[0]) {
      ...
      console.error("reviewActivationRequest failed", err);
      return { ok: false, code: "unavailable", error: "تعذر حفظ قرار طلب التفعيل" };
    }
  });

export const updateLead = createServerFn({ method: "POST" })
  .middleware([authMiddleware])
  .validator(z.object({ id: z.string().min(1).max(100), status: z.enum(LEAD_STATUSES), notes: z.string().trim().max(3000).optional() }))
  .handler(async ({ context, data }): Promise<FnResult<AdminLead>> => {
    const permission = await assertAdmin(context.userId);
    if (!permission.ok) return permission;
    try {
      const sql = await getSql();
      const rows = await sql<Record<string, unknown>>`update leads set status = ${data.status}, notes = ${data.notes ?? ""}, updated_at = now() where id = ${data.id} and activation_status = 'legacy' returning *`;
