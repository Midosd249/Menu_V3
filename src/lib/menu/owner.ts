...
export const updateTenant = createServerFn({ method: "POST" })
  ...
  .validator(
    z.object({
      ...
      logoUrl: z.string().trim().max(450_000).optional(),
      coverUrl: z.string().trim().max(450_000).optional(),
    ...
    try {
      ...
      await sql`
        update tenants set
          ...
          snapchat_url = coalesce(${data.snapchatUrl ?? null}, snapchat_url),
          facebook_url = coalesce(${data.facebookUrl ?? null}, facebook_url),
          tiktok_url = coalesce(${data.tiktokUrl ?? null}, tiktok_url),
          vat_registration_number = coalesce(${data.vatRegistrationNumber ?? null}, vat_registration_number),
...
export const saveProduct = createServerFn({ method: "POST" })
  ...
  .validator(
    z.object({
      ...
      imageUrl: z.string().trim().max(450_000).optional(),
