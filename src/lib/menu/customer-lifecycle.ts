...
const requestSchema = z.object({ nameAr: z.string().trim().min(2).max(80), nameEn: z.string().trim().max(80).optional(), businessType: businessTypeSchema, descriptionAr: z.string().trim().max(160).optional() });

export type CustomerActivationRequest = { id: string; status: ActivationStatus; brandNameAr: string; brandNameEn: string; businessType: BusinessType; descriptionAr: string; contactName: string; contactPhone: string; contactEmail: string; decisionReason: string; requestedAt: string; updatedAt: string; tenantId: string | null; tenantSlug: string | null };
function mapRequest(row: Record<string, unknown>): CustomerActivationRequest { return { id: String(row.id), status: row.activation_status as ActivationStatus, brandNameAr: String(row.business_name ?? ""), brandNameEn: String(row.brand_name_en ?? ""), businessType: String(row.business_type ?? "restaurant") as BusinessType, descriptionAr: String(row.details ?? ""), contactName: String(row.contact_name ?? ""), contactPhone: String(row.contact_phone ?? ""), contactEmail: String(row.contact_email ?? ""), decisionReason: String(row.decision_reason ?? ""), requestedAt: new Date(String(row.activation_requested_at ?? row.created_at)).toISOString(), updatedAt: new Date(String(row.updated_at ?? row.created_at)).toISOString(), tenantId: row.tenant_id ? String(row.tenant_id) : null, tenantSlug: row.tenant_slug ? String(row.tenant_slug) : null }; }

async function account(sql: Awaited<ReturnType<typeof getSql>>, userId: string) {
  const rows = await sql<{ name: string; email: string; phone: string | null }>`select "name", "email", "phoneNumber" as phone from "user" where "id" = ${userId} limit 1`;
  return rows[0] ?? null;
}
...
export const submitActivationRequest = createServerFn({ method: "POST" }).middleware([authMiddleware]).validator(requestSchema).handler(async ({ context, data }): Promise<FnResult<CustomerActivationRequest>> => {
  try {
    const sql = await getSql();
    ...
    if (!/^\+9665\d{8}$/.test(phone)) return { ok: false, code: "invalid", error: "أكمل رقم الجوال السعودي قبل إرسال طلب التفعيل" };
    const existingMember = await sql`select tenant_id from tenant_members where user_id = ${context.userId} and is_active = true order by created_at limit 1`;
    if (existingMember[0]) return { ok: false, code: "conflict", error: "هذا الحساب مرتبط بمساحة عمل بالفعل" };

    const existing = await sql<Record<string, unknown>>`select * from leads where account_user_id = ${context.userId} limit 1`;
    if (existing[0]) {
      ...
      const rows = await sql<Record<string, unknown>>`update leads set business_name = ${data.nameAr}, brand_name_en = ${data.nameEn ?? ""}, business_type = ${data.businessType}, details = ${data.descriptionAr ?? ""}, contact_name = ${currentAccount.name ?? ""}, contact_phone = ${phone}, contact_email = ${currentAccount.email.trim().toLowerCase()}, activation_status = 'pending', activation_requested_at = now(), decision_at = null, decision_by = null, decision_reason = '', status = 'new', source = 'self-serve-registration', updated_at = now() where id = ${String(existing[0].id)} returning *`;
      if (!rows[0]) return { ok: false, code: "unavailable", error: "تعذر تحديث طلب التفعيل" };
      return { ok: true, data: mapRequest(rows[0]) };
    }

    const id = newId();
    const rows = await sql<Record<string, unknown>>`insert into leads (id, business_name, brand_name_en, business_type, city, contact_name, contact_phone, contact_email, details, status, source, account_user_id, activation_status, activation_requested_at) values (${id}, ${data.nameAr}, ${data.nameEn ?? ""}, ${data.businessType}, null, ${currentAccount.name ?? ""}, ${phone}, ${currentAccount.email.trim().toLowerCase()}, ${data.descriptionAr ?? ""}, 'new', 'self-serve-registration', ${context.userId}, 'pending', now()) returning *`;
    return { ok: true, data: mapRequest(rows[0]) };
  } catch (err) {
    if (err && typeof err === "object" && "code" in err && String((err as { code?: unknown }).code) === "23505") {
      try {
        const sql = await getSql();
        const rows = await sql<Record<string, unknown>>`select * from leads where account_user_id = ${context.userId} limit 1`;
        if (rows[0]) return { ok: true, data: mapRequest(rows[0]) };
      } catch (recoveryError) { console.error("submitActivationRequest conflict recovery failed", recoveryError); }
    }
...
export const activateApprovedWorkspace = createServerFn({ method: "POST" }).middleware([authMiddleware]).handler(async ({ context }): Promise<FnResult<{ tenantId: string; slug: string }>> => {
  try {
    ...
    const result = await sql<{ tenant_id: string; slug: string }>`select * from menu_v3.activate_customer_workspace(${request.id}, ${context.userId}, ${tenantId}, ${branchId}, ${slug})`;
