  ...
  .handler(async ({ data }): Promise<FnResult<{ orderId: string; orderNumber: number; total: number; currency: string }>> => {
    try {
      ...
      const products = await sql<ProductRow>`
        ...
        from products
        where tenant_id = ${tenant.id} and id = any(${productIds})
      `;
      ...
      const [variants, options] = await Promise.all([
        variantIds.length
          ? sql<VariantRow>`
              ...
              where tenant_id = ${tenant.id} and id = any(${variantIds})
            `
      ...
      if (variants.length !== variantIds.length || options.length !== modifierOptionIds.length) return fail("أحد خيارات الطلب غير صالح");
      if (variants.some((v) => !v.is_available) || options.some((o) => !o.is_available)) return fail("أحد خيارات الطلب غير متاح");

      const groupIds = [...new Set(options.map((o) => String(o.group_id)))];
      const groups = groupIds.length
        ? await sql<GroupRow>`
            select id, name_ar, name_en, min_select, max_select, is_required
            from modifier_groups
            where tenant_id = ${tenant.id} and id = any(${groupIds}) and is_active = true
          `
        : [];
      const groupById = new Map(groups.map((g) => [String(g.id), g]));
      if (groups.length !== groupIds.length) return fail("مجموعة إضافات غير متاحة");

      const prepared: PreparedItem[] = [];
      for (const item of data.items) {
        const product = productById.get(item.productId)!;
        const variant = item.selected.variantId ? variantById.get(item.selected.variantId) : undefined;
        if (item.selected.variantId && (!variant || String(variant.product_id) !== item.productId)) return fail("الحجم المحدد لا يطابق الصنف");

        const optionRows = item.selected.modifierOptionIds.map((id) => optionById.get(id)).filter((o): o is OptionRow => Boolean(o));
        const uniqueOptionIds = new Set(optionRows.map((o) => String(o.id)));
        if (uniqueOptionIds.size !== optionRows.length) return fail("لا يمكن تكرار الإضافة نفسها");
        if (optionRows.some((o) => !groupById.has(String(o.group_id)))) return fail("إضافة غير صالحة");
        ...
        if (variant) {
          selectedOptions.push({
            ...
            priceDelta: Number(variant.price) - Number(product.price),
          });
        }
        ...
        for (const group of groups) {
          ...
          if (count < Number(group.min_select) || count > Number(group.max_select)) {
            return fail(`خيارات المجموعة «${group.name_ar}» غير مكتملة`);
          }
      ...
      const created = await sql<{ id: string; order_number: number }>`
        with new_order as (
          insert into orders (
            id, tenant_id, branch_id, status, source,
            customer_name, customer_phone, customer_email, notes,
            currency, subtotal, total
          ) values (
            ${orderId}, ${tenant.id}, ${branchId}, 'new', ${data.source},
            ${data.customerName}, ${data.customerPhone}, ${data.customerEmail || null}, ${data.notes ?? null},
            ${tenant.currency || "SAR"}, ${subtotal}, ${subtotal}
          ) returning id, order_number
        ),
        inserted_items as (
          insert into order_items (
            id, order_id, product_id, product_name_ar, product_name_en,
            quantity, unit_price, line_total, selected_options
          )
          select
            ${newId()}, new_order.id, item.product_id, item.product_name_ar, item.product_name_en,
            item.quantity, item.unit_price, item.line_total, item.selected_options
          from new_order
          cross join lateral jsonb_to_recordset(${itemsJson}::jsonb) as item(
            product_id text,
            quantity integer,
            unit_price numeric,
            line_total numeric,
        ...
        inserted_event as (
          ...
          select ${newId()}, id, null, 'new' from new_order
          returning order_id
        )
