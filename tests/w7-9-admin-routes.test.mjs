...
const overviewRoute = await readFile(new URL("../src/routes/admin/index.tsx", import.meta.url), "utf8");
...
test("W7.9 exposes the verified Admin route architecture", () => {
  ...
  assert.match(overviewRoute, /createFileRoute\("\/admin\/"\)/);
...
test("W7.9 generated route tree contains the real Admin child route", () => {
  ...
  assert.match(routeTree, /AdminIndexRouteImport/);
...
test("W7.9 preserves every existing Admin workspace mapping", () => {
  ...
  for (const [tab, path] of Object.entries(mappings)) {
    assert.match(admin, new RegExp(`${tab}: \\"${path.replaceAll("/", "\\/")}\\"`));
  }
});

test("W7.9 keeps legacy tab compatibility and safe unknown fallback", () => {
  for (const tab of ["overview", "tenants", "orders", "clients", "branches", "leads", "projects", "requests", "subscriptions", "analytics", "activity", "system"]) {
    assert.match(admin, new RegExp(`${tab}: \\"/admin`));
  }
...
test("W7.9 child route whitelists only real Admin workspaces", () => {
  for (const workspace of ["restaurants", "orders", "clients", "branches", "leads", "projects", "service-requests", "subscriptions", "analytics", "activity", "system"]) {
    assert.match(workspaceRoute, new RegExp(`\\"${workspace}\\"`));
  }
