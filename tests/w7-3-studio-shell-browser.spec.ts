...
async function assertMidnightInkAndSand(page: import("playwright/test").Page) {
  const visual = await page.evaluate(() => {
    ...
    return { studioScope: Boolean(document.querySelector('nav[aria-label="مساحات العمل"]')), bodyBackground: body.backgroundColor, bodyPaper: body.getPropertyValue("--color-paper").trim(), bodyInk: body.getPropertyValue("--color-ink").trim(), bodyRing: body.getPropertyValue("--color-ring").trim(), canvasBackground: canvas ? getComputedStyle(canvas).backgroundColor : "", headerBackground: header ? getComputedStyle(header).backgroundColor : "", asideBackground: aside ? getComputedStyle(aside).backgroundColor : "", activeBackground: active ? getComputedStyle(active).backgroundColor : "", activeColor: active ? getComputedStyle(active).color : "" };
  });
...
test("customer approval lifecycle browser QA covers request, decisions, activation, RTL and LTR", async ({ page }) => {
  ...
  await writeFile(fixturePath, `delete from tenant_members where user_id = 'dev-user';
...
`);
  const child = spawn("node", ["scripts/with-app-env.mjs", "./node_modules/vite/bin/vite.js", "--host", "127.0.0.1", "--port", port], { env: { ...process.env, VITE_AUTH_ENABLED: "false", PLATFORM_ADMIN_USER_IDS: "dev-user", DATABASE_URL: "", SUPABASE_DB_URL: "", POSTGRES_URL: "", POSTGRES_PRISMA_URL: "", POSTGRES_URL_NON_POOLING: "" }, stdio: "ignore" });
  try {
    let started = false;
    for (let attempt = 1; attempt <= 120; attempt += 1) {
      ...
      } catch (error) { if (attempt === 120) throw error; }
