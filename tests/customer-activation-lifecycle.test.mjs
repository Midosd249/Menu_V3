...
test("customer activation lifecycle blocks signup-only tenant creation and is idempotent", { skip: !databaseUrl }, async () => {
  ...
  try {
    ...
    await pool.query(`
      ...
      create table ${schema}.leads (id text primary key, business_name text not null, brand_name_en text not null default '', business_type text not null default 'restaurant', city text, contact_name text not null default '', contact_phone text not null default '', contact_email text not null default '', details text, status text not null default 'new', notes text not null default '', source text not null default 'website', created_at timestamptz not null default now(), updated_at timestamptz not null default now(), account_user_id text, activation_status text not null default 'legacy', activation_requested_at timestamptz, decision_at timestamptz, decision_by text, decision_reason text not null default '');
      ...
      create table ${schema}.tenants (id text primary key, owner_user_id text not null, slug text not null unique, name_ar text not null, name_en text not null default '', tagline_ar text not null default '', business_type text not null, is_published boolean not null default false, is_active boolean not null default true);
    ...
    await assert.rejects(pool.query(`insert into ${schema}.tenants (id, owner_user_id, slug, name_ar, name_en, business_type) values ($1,$2,$3,$4,$5,$6)`, [randomUUID(), userId, "signup-only", "Signup Only", "", "restaurant"]), /CUSTOMER_APPROVAL_REQUIRED/, "signup alone must not authorize tenant creation");
    ...
    assert.equal(decisions.filter((result) => result.status === "fulfilled" && result.value.rowCount === 1).length, 1, "exactly one concurrent admin decision must win");
...
test("legacy approval decision is serialized in the database", { skip: !databaseUrl }, async () => {
  const pool = new Pool({ connectionString: databaseUrl, max: 6 });
  const schema = `legacy_approval_${randomUUID().replaceAll("-", "")}`;
  const leadId = randomUUID();
  try {
    await pool.query(`create schema ${schema}`);
    await pool.query(`create table ${schema}.leads (id text primary key, business_name text not null, status text not null default 'new', updated_at timestamptz not null default now()); create table ${schema}.lead_onboarding (id text primary key, lead_id text, token_hash text, expires_at timestamptz, approved_at timestamptz, used_at timestamptz, revoked_at timestamptz, tenant_id text, created_by text);`);
    await pool.query(isolateMigration(approvalMigration, schema));
    await pool.query(`insert into ${schema}.leads (id,business_name) values ($1,$2)`, [leadId, "Legacy Brand"]);
    const approve = (suffix) => pool.query(`select * from ${schema}.approve_legacy_lead($1,$2,$3,$4)`, [leadId, `hash-${suffix}`, new Date(Date.now() + 86400000).toISOString(), `admin-${suffix}`]);
    const results = await Promise.allSettled([approve("a"), approve("b")]);
    assert.equal(results.filter((result) => result.status === "fulfilled").length, 1, "only one concurrent legacy approval may create an active token");
    const rows = await pool.query(`select count(*)::int as count from ${schema}.lead_onboarding where lead_id=$1 and used_at is null and revoked_at is null`, [leadId]);
