...
test("customer activation lifecycle blocks signup-only tenant creation and is idempotent", { skip: !databaseUrl }, async () => {
  ...
  try {
    ...
    await pool.query(`
      ...
      create table ${schema}.tenants (id text primary key, owner_user_id text not null, slug text not null unique, name_ar text not null, name_en text not null default '', tagline_ar text not null, business_type text not null, is_published boolean not null default false, is_active boolean not null default true);
