...
test("Heritage category products place price on its own row below the description", async () => {
  const styles = await readFile("src/theme-heritage-hardening.css", "utf8");
  assert.match(styles, /editorial-product-card[\s\S]*grid-template-rows:\s*auto auto auto auto/);
  assert.match(styles, /editorial-product-image[\s\S]*grid-row:\s*1 \/ span 4/);
  assert.match(styles, /editorial-product-topline[\s\S]*grid-row:\s*1/);
