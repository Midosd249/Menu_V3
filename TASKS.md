...
- VERIFIED: Quality #2313 / run `35766577618` passed.
...
- VERIFIED: the Phase 6 golden 30-product performance fixture remained green.
- VERIFIED: no Phase 1–6 reimplementation or protected architecture changes were introduced.
- UNKNOWN: real-device Android/iOS LCP/waterfall and Production performance for the real 30-item customer reproduction.
- BLOCKED / NON-BLOCKING: Vercel preview remains rate-limited; no retry/deployment was performed.

## EXACT NEXT TASK

**Phase 8 — Production + Real Device QA.**

1. Follow the release-only sequence.
2. Perform one authorized Production deployment from verified `main`.
3. Verify the deployed commit SHA directly.
4. Run Android/iOS/QR QA for all five themes.
5. Verify Arabic RTL and English LTR.
6. Measure Studio/public-menu image waterfall and LCP at 320/375/390/430px where the device/tooling exposes them.
7. Verify product details, cart/order, Featured, cover/logo, fallback media, and configured customer actions.
8. Record exact deployment/device evidence and stop.

Do not re-implement Phase 1–7 without new measured regression evidence.
