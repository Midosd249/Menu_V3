...
test("public tenant mapping has a distinct type that cannot carry owner identity", () => {
  ...
  assert.match(types, /export type PublicTenant = Omit<Tenant, "ownerUserId" \| "isPublished" \| "isActive" \| "createdAt" \| "updatedAt">;/); assert.match(types, /export type PublicMenu = \{ tenant: PublicTenant;/); assert.match(map, /export function mapPublicTenant\(row: Record<string, unknown>\): PublicTenant/); assert.match(map, /id: tenant\.id/); assert.doesNotMatch(map, /\.\.\.publicTenant/); assert.match(publicMenu, /const tenant = mapPublicTenant\(row\.tenant\);/); assert.doesNotMatch(publicMenu, /const tenant = mapTenant\(row\.tenant\);/);
});
...
test("authenticated menu server functions use the auth middleware chokepoint", () => {
  for (const path of ["src/lib/menu/owner.ts", "src/lib/menu/admin.ts"]) {
    const source = read(path); const blocks = source.split(/(?=export const \w+ = createServerFn)/g).filter((block) => block.startsWith("export const ")); assert.ok(blocks.length > 0, `${path} contains no server functions`);
    for (const block of blocks) { const name = block.match(/^export const (\w+)/)?.[1] ?? "unknown"; const handlerIndex = block.indexOf(".handler("); assert.ok(handlerIndex > 0, `${path}:${name} has no handler`); assert.match(block.slice(0, handlerIndex), /\.middleware\(\[authMiddleware\]\)/, `${path}:${name} is missing auth middleware`); }
  }
});

test("platform administration is fail-closed and cannot be inferred from a client-supplied tenant", () => {
  const admin = read("src/lib/menu/admin.ts"); const guard = read("src/lib/auth/platform-admin.server.ts");
  assert.match(admin, /requirePlatformAdmin\(context\.userId\)/); assert.match(guard, /menu_v3\.is_platform_admin\(\$\{userId\}\)/); assert.match(guard, /isPlatformAdminConfigured\(userId, users\[0\]\?\.email\)/); assert.match(guard, /throw new Error\("PLATFORM_ADMIN_REQUIRED"\)/); assert.doesNotMatch(admin, /platformAdmin.*tenantId|tenantId.*platformAdmin/i);
});
