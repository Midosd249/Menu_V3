...
const noPublicTenantMutation = new RegExp("(^|\\s)(alter table|update|create trigger|drop trigger) tenants\\b", "m");
const noUnqualifiedContentTriggers = new RegExp("(^|\\s)(create trigger|drop trigger) (branches|branch_hours|categories|products|product_variants|modifier_groups|modifier_options|product_modifier_groups)\\b", "m");
...
test("public content revision migration is explicitly scoped to menu_v3", () => {
  ...
  assert.doesNotMatch(migrationSource, noPublicTenantMutation);
  assert.doesNotMatch(migrationSource, noUnqualifiedContentTriggers);
});
